...
///modelling of soil behaviour -ruofeng
typedef struct{
  float coh;					///<EP model cohesion
  float phi;					///<EP model friction angle
  float dlt;          ///<EP model dilation angle
  float phi_r;        ///<Strength softening
...
typedef enum{
  ...
  FST_NearFreeSurface=1, ///<Interior particle close enough to free surface to have truncated kernel support.
...
typedef enum {
    ...
    DP_MC=2,     ///<3D Mid Circumscribed DP
    DP_C=1      ///<3D Circumscribed DP
}TpDPCtes;
