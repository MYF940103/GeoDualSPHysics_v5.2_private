...
void JSph::InitVars(){
  ...
  CurvedDrainedMLSOrder=1;
  CurvedDrainedMLSRadiusFactor=1.;
  ...
  CurvedDrainedCorrectedLapRadiusFactor=2.;
  CurvedDrainedCorrectedLapRMinFactor=0.7;
  CurvedDrainedCorrectedLapBoundarySamples=9;
  CurvedDrainedCorrectedLapMinSamples=12;
  CurvedDrainedCorrectedLapConditionLimit=1.e12;
  ...
  PorePressureFeedbackDiagRawMean=0.;
  PorePressureFeedbackDiagUsedMax=0.;
  PorePressureFeedbackDiagUsedMean=0.;
  PorePressureFeedbackDiagPreMax=0.;
  PorePressureFeedbackDiagRatioMax=0.;
  PorePressureFeedbackDiagCapValueMin=0.;
  PorePressureFeedbackDiagConfiningRef=0.;
...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  //-DP configuration.
  switch (eparms.GetValueInt("DPCtes",true,1)) {
    case 1:  DPCtes=DP_C;   break;
    case 2:  DPCtes=DP_MC;  break;
  ...
  switch(eparms.GetValueInt("CurvedDrainedBoundaryMode",true,0)){
    ...
    case 8:  CurvedDrainedBoundaryMode=8;  break;
    default: Run_Exceptioon("CurvedDrainedBoundaryMode is not valid. Valid values are 0, 1, diagnostic-only 2, experimental 3, experimental 4, experimental 5, experimental 6, experimental 7, and experimental 8.");
  }
  ...
  switch(eparms.GetValueInt("CurvedDrainedMLSOrder",true,1)){
    case 0:  CurvedDrainedMLSOrder=0;  break;
    case 1:  CurvedDrainedMLSOrder=1;  break;
    default: Run_Exceptioon("CurvedDrainedMLSOrder is not valid. Valid values are 0 and 1.");
  }
  CurvedDrainedMLSRadiusFactor=eparms.GetValueDouble("CurvedDrainedMLSRadiusFactor",true,1.);
  switch(eparms.GetValueInt("CurvedDrainedFluxDiagnostics",true,0)){
    case 0:  CurvedDrainedFluxDiagnostics=false;  break;
    case 1:  CurvedDrainedFluxDiagnostics=true;   break;
    default: Run_Exceptioon("CurvedDrainedFluxDiagnostics mode is not valid.");
  }
  CurvedDrainedMLSConditionLimit=eparms.GetValueDouble("CurvedDrainedMLSConditionLimit",true,1.e8);
  switch(eparms.GetValueInt("CurvedDrainedMLSFallbackMode",true,3)){
  ...
  CurvedDrainedCorrectedLapRadiusFactor=eparms.GetValueDouble("CurvedDrainedCorrectedLapRadiusFactor",true,2.);
  CurvedDrainedCorrectedLapRMinFactor=eparms.GetValueDouble("CurvedDrainedCorrectedLapRMinFactor",true,0.7);
  {
    const int nsamp=eparms.GetValueInt("CurvedDrainedCorrectedLapBoundarySamples",true,9);
    if(nsamp<1)Run_Exceptioon("CurvedDrainedCorrectedLapBoundarySamples must be greater than zero.");
    CurvedDrainedCorrectedLapBoundarySamples=unsigned(nsamp);
  }
  {
    const int nsamp=eparms.GetValueInt("CurvedDrainedCorrectedLapMinSamples",true,12);
    if(nsamp<10)Run_Exceptioon("CurvedDrainedCorrectedLapMinSamples must be at least 10.");
    CurvedDrainedCorrectedLapMinSamples=unsigned(nsamp);
  }
  CurvedDrainedCorrectedLapConditionLimit=eparms.GetValueDouble("CurvedDrainedCorrectedLapConditionLimit",true,1.e12);
  CurvedDrainedCorrectedLapBoundaryWeight=eparms.GetValueDouble("CurvedDrainedCorrectedLapBoundaryWeight",true,1.);
  switch(eparms.GetValueInt("CurvedDrainedCorrectedLapFallbackMode",true,0)){
    case 0:  CurvedDrainedCorrectedLapFallbackMode=0;  break;
    case 4:  CurvedDrainedCorrectedLapFallbackMode=4;  break;
    default: Run_Exceptioon("CurvedDrainedCorrectedLapFallbackMode is not valid. Valid values are 0 keep material operator and 4 local gap fallback.");
  }
  switch(eparms.GetValueInt("CurvedDrainedCorrectedLapDiagnostics",true,0)){
    case 0:  CurvedDrainedCorrectedLapDiagnostics=false;  break;
  ...
  switch(eparms.GetValueInt("SavePorePressureFeedbackDiagnostics",true,0)){
    ...
    default: Run_Exceptioon("SavePorePressureFeedbackDiagnostics mode is not valid.");
  }
  const int ppfbdiaginterval=eparms.GetValueInt("PorePressureFeedbackDiagInterval",true,1);
  PorePressureFeedbackDiagInterval=(ppfbdiaginterval>0? unsigned(ppfbdiaginterval): 0u);
  switch(eparms.GetValueInt("PorePressureFeedbackLimiterMode",true,0)){
    case 0:  PorePressureFeedbackLimiterMode=0;  break;
    case 1:  PorePressureFeedbackLimiterMode=1;  break;
    case 2:  PorePressureFeedbackLimiterMode=2;  break;
    case 3:  PorePressureFeedbackLimiterMode=3;  break;
    default: Run_Exceptioon("PorePressureFeedbackLimiterMode is not valid. Valid values are 0:none, 1:absolute cap, 2:ratio cap, 3:absolute+ratio cap.");
  }
  ...
  if(PorePressureFeedbackDiagInterval<1)
    Run_Exceptioon("PorePressureFeedbackDiagInterval must be greater than zero.");
  if(PorePressureFeedbackRelaxation<0. || PorePressureFeedbackRelaxation>1.)
    Run_Exceptioon("PorePressureFeedbackRelaxation must be between 0 and 1. Use 0 to disable relaxation.");
  if(PorePressureFeedbackMaxAccel<0.)
  ...
  if(PorePressureCurvedDrained && CurvedDrainedBoundaryMode==5 && CurvedDrainedMLSRadiusFactor<0.)
  ...
  if(PorePressureCurvedDrained && CurvedDrainedBoundaryMode==8 && CurvedDrainedCorrectedLapRadiusFactor<0.)
    Run_Exceptioon("CurvedDrainedCorrectedLapRadiusFactor must be greater than or equal to zero.");
  if(PorePressureCurvedDrained && CurvedDrainedBoundaryMode==8 && (CurvedDrainedCorrectedLapRMinFactor<0. || CurvedDrainedCorrectedLapRMinFactor>1.))
    Run_Exceptioon("CurvedDrainedCorrectedLapRMinFactor must be between 0 and 1.");
  if(PorePressureCurvedDrained && CurvedDrainedBoundaryMode==8 && CurvedDrainedCorrectedLapConditionLimit<=0.)
...
void JSph::LoadCaseConfig(const JSphCfgRun *cfg){
  ...
  InitSoilParameters(&xml,"case.execution.special.soils");
  ConfigConstantsSoil();
...
void JSph::VisuConfig(){
  ...
  if(HydromechCoupling){
    ...
    if(PorePressureBoundaryOperator==3){
      ...
      Log->Print(fun::VarStr("  CurvedDrainedMLSOrder",CurvedDrainedMLSOrder));
      Log->Print(fun::VarStr("  CurvedDrainedMLSRadiusFactor",CurvedDrainedMLSRadiusFactor));
      ...
      Log->Print(fun::VarStr("  CurvedDrainedCorrectedLapRadiusFactor",CurvedDrainedCorrectedLapRadiusFactor));
      Log->Print(fun::VarStr("  CurvedDrainedCorrectedLapRMinFactor",CurvedDrainedCorrectedLapRMinFactor));
      Log->Print(fun::VarStr("  CurvedDrainedCorrectedLapBoundarySamples",CurvedDrainedCorrectedLapBoundarySamples));
      Log->Print(fun::VarStr("  CurvedDrainedCorrectedLapMinSamples",CurvedDrainedCorrectedLapMinSamples));
      Log->Print(fun::VarStr("  CurvedDrainedCorrectedLapConditionLimit",CurvedDrainedCorrectedLapConditionLimit));
      ...
      if(CurvedDrainedBoundaryMode==8)
        Log->Print("  CurvedDrainedBoundaryMode=8: boundary-aware corrected quadratic MLS Laplacian; CPU-only experimental.");
...
void JSph::AddBasicArrays(JDataArrays &arrays,unsigned np,const tdouble3 *pos
  ...
  ,const tfloat3 *sigma_kk,const tfloat3 *sigma_ij, const float *kplastic)const
{
...
//==============================================================================
/// Returns whether opt-in pore-pressure feedback stabilization is enabled.
//==============================================================================
...
//==============================================================================
/// Prints a compact CPU pore-pressure feedback diagnostics line.
//==============================================================================
void JSph::PrintPorePressureFeedbackDiagnostics()const{
  ...
  if(Nstep<5 || !(Nstep%PorePressureFeedbackDiagInterval)){
    Log->Printf("PorePressureFeedback diagnostics: step=%d, TimeStep=%g, factor=%g, applied=%u, class_skipped=%u, raw_max=%g, raw_mean=%g, used_max=%g, used_mean=%g, pre_accel_max=%g, used_to_reference_ratio_max=%g, confining_ref=%g, limited=%u, relaxed=%u, cap_min=%g."
      ...
      ,PorePressureFeedbackDiagConfiningRef,PorePressureFeedbackDiagLimitedCount
      ,PorePressureFeedbackDiagRelaxedCount,PorePressureFeedbackDiagCapValueMin);
    if(ConfiningStressGeometry==1){
...
//==============================================================================
/// Returns name of DP constants for DP constitutive model in text format.
//==============================================================================
std::string JSph::GetDPName(TpDPCtes dpctes){
    string tx;
    if (dpctes==DP_C)tx = "3D DP Circumscribed";
    else if (dpctes==DP_MC)tx = "3D DP Mid circumscribed";
    else if (dpctes==DP_PS)tx = "Plain Strain DP";
    else tx = "???";
    return(tx);
}
//==============================================================================
...
//##############################################################################
//# Soil modelling: Variables and methods.  
//##############################################################################
...
void JSph::InitSoilParameters(const JXml *sxml,std::string xmlpath){
  ...
  if(SoilCte.SoilConstitutiveModel==3){
    ...
    if(solidNode->FirstChildElement("MccSubstepping")){
      const int v=sxml->ReadElementInt(solidNode,"MccSubstepping","value",false);
      if(v!=0 && v!=1)Run_Exceptioon("MccSubstepping must be 0 or 1.");
      SoilCte.MccSubstepping=(v==1);
    }
  ...
  const StSoilCte &ct=SoilCte;
   ...
   if(ct.SoilConstitutiveModel==0)
     ...
     else if(ct.MccAdmissibilityGuard)
       Log->Print("  MCC admissibility guard: enabled for single-step return checks.");
