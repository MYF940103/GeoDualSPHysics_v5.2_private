...
void JSphCpuSingle::ConfigDomain(){
  ...
  //=========
  if((HydromechCoupling || SavePorePressure) && (!PorePressc || !PorePressRatec || !DivVelc || !LapPorePressc || !LapZc || !PorePressureAcec || !PorePressureAceDiffc || !PorePressureFeedbackUsedAcec)){
  ...
  if(LapPorePressc){
    memset(LapPorePressc,0,sizeof(float)*Np);
    Log->Printf("Hydromechanical diagnostic field initialised on CPU: LapPorePress=0 for %u material particles.",Np-Npb);
  }
  if(LapZc){
  ...
  if(PorePressureFeedbackUsedAcec){
    memset(PorePressureFeedbackUsedAcec,0,sizeof(tfloat3)*Np);
    Log->Printf("Pore-pressure feedback used acceleration memory initialised on CPU: PorePressureFeedbackUsedAccel=(0,0,0) for %u material particles.",Np-Npb);
  }
  ...
  if(HydromechCoupling && PorePressureModel==1 && PorePressureFeedback)
    Log->Printf("Pore-pressure feedback to momentum: enabled on CPU. Feedback mode=%s, operator=%s."
      ...
      ,(PorePressureFeedbackOperator==3? "paper-style stress-pair": (PorePressureFeedbackOperator==2? "LSQ pressure-gradient": (PorePressureFeedbackOperator==1? "difference-gradient": "symmetric stress-style"))));
  ...
  if(SoilCte.SoilConstitutiveModel==3 || SoilCte.SaveMccState){
    ...
    memset(MccSubstepCountc,0,sizeof(float)*Np);
  ...
  const double hydrotimestepini=(PartBeginFirst? PartsLoaded->GetPartBeginTimeStep(): 0.);
  if(HydromechCoupling && PorePressureModel==1 && PorePressureTopDrained && PorePressc){
    if(hydrotimestepini>=PorePressureTopDrainedStartTime)ApplyPorePressureTopDrained(Np-Npb,Npb,Posc,Codec,PorePressc,hydrotimestepini,"initialization",true);
    else Log->Printf("Top drained pore-pressure boundary is inactive at initialization: TimeStep=%g, start_time=%g.",hydrotimestepini,PorePressureTopDrainedStartTime);
  }
...
void JSphCpuSingle::PeriodicDuplicateVerlet(unsigned np,unsigned pini,tuint3 cellmax
  ...
  ,tdouble3 *pos,tfloat4 *velrhop,tsymatrix3f *spstau,double *porepress,float *porepressrate,float *divvel,float *lapporepress,float *lapz,tfloat3 *porepressureace,tfloat3 *porepressureacediff,tfloat3 *porepressurefeedbackused,double *porepressghost,double *excessporepressghost,float *porepressureboundarymode,float *lapporepressghost,float *lapzghost,tfloat4 *velrhopm1,tsymatrix3f *sigma,tsymatrix3f *sigmam1)const
{
  const int n=int(np);
  #ifdef OMP_USE
  ...
  for(int p=0;p<n;p++){
    ...
    if(porepressurefeedbackused)porepressurefeedbackused[pnew]=porepressurefeedbackused[pcopy];
    ...
    if(lapporepressghost)lapporepressghost[pnew]=lapporepressghost[pcopy];
    if(lapzghost)lapzghost[pnew]=lapzghost[pcopy];
  ...
  ,unsigned *idp,typecode *code,unsigned *dcell,tdouble3 *pos,tfloat4 *velrhop,tsymatrix3f *spstau,double *porepress,float *porepressrate,float *divvel,float *lapporepress,float *lapz,tfloat3 *porepressureace,tfloat3 *porepressureacediff,tfloat3 *porepressurefeedbackused,double *porepressghost,double *excessporepressghost,float *porepressureboundarymode,float *lapporepressghost,float *lapzghost,tdouble3 *pospre,tfloat4 *velrhoppre,tsymatrix3f *sigma, tsymatrix3f *sigmapre)const
{
  ...
  for(int p=0;p<n;p++){
    ...
    if(spstau)spstau[pnew]=spstau[pcopy];
    if(sigmapre)sigmapre[pnew] = sigmapre[pcopy];//mdbr
  }
}
      ...
      for(unsigned cblock=0;cblock<2;cblock++){//-0:new periodic, 1:original particles. | 0:periodicas nuevas, 1:particulas originales
        ...
        while(run && num2){
          ...
          if(count>nmax || !enoughmemory){
            ...
            if(TStep==STEP_Verlet)PeriodicDuplicateVerlet(count,Np,DomCells,perinc,listp,Idpc,Codec,Dcellc,Posc,Velrhopc,SpsTauc,PorePressc,PorePressRatec,DivVelc,LapPorePressc,LapZc,PorePressureAcec,PorePressureAceDiffc,PorePressureFeedbackUsedAcec,PorePressGhostc,ExcessPorePressGhostc,PorePressureBoundaryModec,LapPorePressGhostc,LapZGhostc,VelrhopM1c,Sigmac,SigmaM1c);
            if(TStep==STEP_Symplectic){
              if((PosPrec || VelrhopPrec) && (!PosPrec || !VelrhopPrec))Run_Exceptioon("Symplectic data is invalid.") ;
              PeriodicDuplicateSymplectic(count,Np,DomCells,perinc,listp,Idpc,Codec,Dcellc,Posc,Velrhopc,SpsTauc,PorePressc,PorePressRatec,DivVelc,LapPorePressc,LapZc,PorePressureAcec,PorePressureAceDiffc,PorePressureFeedbackUsedAcec,PorePressGhostc,ExcessPorePressGhostc,PorePressureBoundaryModec,LapPorePressGhostc,LapZGhostc,PosPrec,VelrhopPrec,Sigmac,SigmaPrec);
            }
...
void JSphCpuSingle::RunCellDivide(bool updateperiodic){
  ...
  if(MccSubstepCountc)CellDivSingle->SortArray(MccSubstepCountc);
  ...
  if(PorePressureFeedbackUsedAcec)CellDivSingle->SortArray(PorePressureFeedbackUsedAcec);
  ...
  if(npfout){
    ...
    //========= mdbr
    tfloat3* sigmakk=ArraysCpu->ReserveFloat3();
    tfloat3* sigmaij=ArraysCpu->ReserveFloat3();
    float* kplastic=ArraysCpu->ReserveFloat();
    //=========
    unsigned num=GetParticlesData(npfout,Np,false,idp,pos,vel,rhop,sigmakk,sigmaij,kplastic,code);
...
void JSphCpuSingle::AbortBoundOut(){
  ...
  float* kplastic=ArraysCpu->ReserveFloat();
  //=========
  GetParticlesData(nboundout,Np,false,idp,pos,vel,rhop,sigmakk,sigmaij,kplastic,code);
...
void JSphCpuSingle::Interaction_Forces(TpInterStep interstep){
  ...
  if(HydromechCoupling && PorePressureModel==1 && (PorePressureBoundaryOperator==1 || PorePressureBoundaryOperator==2 || PorePressureBoundaryOperator==3) && PorePressc && LapPorePressc && LapZc){
    const bool fluxdiag=(PorePressureBoundaryOperator==3 && (CurvedDrainedBoundaryMode==5 || CurvedDrainedBoundaryMode==6 || CurvedDrainedBoundaryMode==7 || CurvedDrainedBoundaryMode==8)
      && (CurvedDrainedFluxDiagnostics || (CurvedDrainedBoundaryMode==7 && CurvedDrainedShellDiagnostics) || (CurvedDrainedBoundaryMode==8 && CurvedDrainedCorrectedLapDiagnostics)));
    const bool printop=(!PorePressureBoundaryOperatorPrint || fluxdiag);
    ApplyPorePressureBoundaryOperator(Np-Npb,Npb,DivData,Dcellc,Posc,Velrhopc,Codec,BoundNormalc,PorePressc,LapPorePressc,LapZc,TimeStep,printop);
    if(!PorePressureBoundaryOperatorPrint)
  ...
  if(HydromechCoupling && PorePressc && PorePressureAcec){
    if(PorePressureFeedbackOperator==3)ComputePorePressureAccelPaper(Np-Npb,Npb,DivData,Dcellc,Posc,Velrhopc,Codec,PorePressc,PorePressureAcec);
    else ComputePorePressureAccel(Np-Npb,Npb,DivData,Dcellc,Posc,Velrhopc,Codec,PorePressc,PorePressureAcec);
  }
  if(HydromechCoupling && PorePressc && PorePressureAceDiffc && (PorePressureFeedbackOperator==1 || PorePressureFeedbackOperator==2)){
...
void JSphCpuSingle::MdbcBoundCorrection(){
  ...
  Interaction_MdbcCorrection(SlipMode,DivData,Posc,Codec,Idpc,BoundNormalc,MotionVelc,Velrhopc,Sigmac);
...
void JSphCpuSingle::CdbcBoundCorrection(){
  Timersc->TmStart(TMC_CfPreForces);
  Interaction_CdbcCorrection(SlipMode,DivData,Posc,Codec,Idpc,BoundNormalc,MotionVelc,Velrhopc,Sigmac);
  Timersc->TmStop(TMC_CfPreForces);
}
...
void JSphCpuSingle::RunGaugeSystem(double timestep,bool saveinput){
  if(!Nstep || GaugeSystem->GetCount()){
    ...
    GaugeSystem->CalculeCpu(timestep,DivData,NpbOk,Npb,Np
      ,Posc,Codec,Idpc,Velrhopc,Sigmac,saveinput);
...
void JSphCpuSingle::SaveData(){
  ...
  if(save){
    ...
    if(SoilCte.SaveMccState){
      try{
        ...
        if(MccSubstepCountc)mccsubstepcount=new float[npsave];
    ...
    if(PorePressureBoundaryGhost && PorePressc && PorePressGhostc && ExcessPorePressGhostc && PorePressureBoundaryModec){
      ...
      if(LapPorePressGhostc)ComputeHydroLapPorePressGhost(Np-Npb,Npb,DivData,Dcellc,Posc,Velrhopc,Codec,PorePressc,PorePressGhostc,PorePressureBoundaryModec,LapPorePressGhostc);
  ...
  if(SoilCte.SaveMccState && mccsubstepcount)arrays.AddArray("MccSubstepCount",npsave,mccsubstepcount);
