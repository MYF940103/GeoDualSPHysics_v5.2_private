  ...
  ,bool saveinput)
{
  ...
  for(unsigned cg=0;cg<ng;cg++){
    ...
    if(gau->Update(timestep)){
      gau->CalculeCpu(timestep,dvd,npbok,npb,np,pos,code,idp,velrhop,sigma);
    }
  }
  //-Saves input state.
...
void JGaugeSystem::CalculeLastInputCpu(std::string gaugename){
  ...
  Gauges[idx]->CalculeCpu(s.timestep,s.dvd,s.npbok,s.npb,s.np,s.pos,s.code,s.idp,s.velrhop,NULL);
}
  ...
  ,bool saveinput)
{
  ...
  for(unsigned cg=0;cg<ng;cg++){
    JGaugeItem* gau=Gauges[cg];
    if(gau->Update(timestep)){
      gau->CalculeGpu(timestep,dvd,npbok,npb,np,posxy,posz,code,idp,velrhop,sigma,AuxMemoryg);
    }
  }
  //-Saves input state.
...
void JGaugeSystem::CalculeLastInputGpu(std::string gaugename){
  ...
  Gauges[idx]->CalculeGpu(s.timestep,s.dvd,s.npbok,s.npb,s.np,s.posxy,s.posz,s.code,s.idp,s.velrhop,NULL,AuxMemoryg);
}
#endif
