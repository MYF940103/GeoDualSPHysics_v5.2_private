...
template<TpKernel tker> __global__ void KerInteractionGaugeForce(float bker
  ...
  ,const float4 *velrhop,const float2 *sigma,float3 *partace)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    ...
    if(CODE_GetTypeAndValue(code1)==codesel && CODE_IsNormal(code1)){
	  ...
	  float2 sigmap1_xx_xy=sigma[p*3];
 	  float2 sigmap1_xz_yy=sigma[p*3+1];
	  float2 sigmap1_yz_zz=sigma[p*3+2];
      ...
      for(int c3=ini3;c3<fin3;c3+=nc.w)for(int c2=ini2;c2<fin2;c2+=nc.x){
        ...
        if(pfin)for(int p2=pini;p2<pfin;p2++){
          ...
          if(rr2<=kernelsize2 && rr2>=ALMOSTZERO && CODE_IsFluid(code[p2])){
            ...
            //const float prs=(press1+press2)/(rhop1*rhop2);
            const float prsxx = mass2*(sigmap1_xx_xy.x + sigmap2_xx_xy.x) / (rhop1*rhop2);
		    const float prsyy = mass2*(sigmap1_xz_yy.y + sigmap2_xz_yy.y) / (rhop1*rhop2);
		    const float prszz = mass2*(sigmap1_yz_zz.y + sigmap2_yz_zz.y) / (rhop1*rhop2);
		    const float prsxy = mass2*(sigmap1_xx_xy.y + sigmap2_xx_xy.y) / (rhop1*rhop2);
		    const float prsxz = mass2*(sigmap1_xz_yy.x + sigmap2_xz_yy.x) / (rhop1*rhop2);
		    const float prsyz = mass2*(sigmap1_yz_zz.x + sigmap2_yz_zz.x) / (rhop1*rhop2);
            {//-Adds aceleration.
...
void Interaction_GaugeForce(const StCteSph &CSP,const StDivDataGpu &dvd
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,const tsymatrix3f *sigma
  ,float3 *partace)
{
    ...
    switch(CSP.tkernel){
      ...
      case KERNEL_Wendland:{ const float bker=CSP.kwend.bwen;
        KerInteractionGaugeForce<KERNEL_Wendland> <<<sgrid,bsize>>>(bker,n,idbegin,codesel
          ...
          ,posxy,posz,code,idp,velrhop,(const float2*)sigma,partace);
      }break;
