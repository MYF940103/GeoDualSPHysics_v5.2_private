...
void JGaugeVelocity::CalculeCpu(double timestep,const StDivDataCpu &dvd
  ...
  ,const typecode *code,const unsigned *idp,const tfloat4 *velrhop,const tsymatrix3f *sigma)
{
  switch(CSP.tkernel){
...
void JGaugeVelocity::CalculeGpu(double timestep,const StDivDataGpu &dvd
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,const tsymatrix3f *sigma,float3 *aux)
{
  SetTimeStep(timestep);
  //-Start measure.
  tfloat3 ptvel=TFloat3(0);
    ...
    for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
      ...
      for(unsigned p2=pif.x;p2<pif.y;p2++){
        ...
        if(dr.w<=CSP.kernelsize2 && dr.w>=ALMOSTZERO && CODE_IsFluid(code[p2])){
          ...
          //const float p_vpm1=-prs*massp2;
          const float prsxx = massp2*(sigmap1.xx + sigmap2.xx) / (velrhop1.w*velrhop2.w);
		  const float prsyy = massp2*(sigmap1.yy + sigmap2.yy) / (velrhop1.w*velrhop2.w);
		  const float prszz = massp2*(sigmap1.zz + sigmap2.zz) / (velrhop1.w*velrhop2.w);
		  const float prsxy = massp2*(sigmap1.xy + sigmap2.xy) / (velrhop1.w*velrhop2.w);
		  const float prsxz = massp2*(sigmap1.xz + sigmap2.xz) / (velrhop1.w*velrhop2.w);
		  const float prsyz = massp2*(sigmap1.yz + sigmap2.yz) / (velrhop1.w*velrhop2.w);
          //ace.x+=p_vpm1*frx;  ace.y+=p_vpm1*fry;  ace.z+=p_vpm1*frz;
          ace.x +=(prsxx*frx+prsxy*fry+prsxz*frz); ace.y+=(prsyy*fry+prsxy*frx+prsyz*frz); ace.z+=(prszz*frz+prsyz*fry+prsxz*frx);
  ...
  ,const typecode *code,const unsigned *idp,const tfloat4 *velrhop,const tsymatrix3f *sigma)
{
  switch(CSP.tkernel){
    case KERNEL_Cubic:       //Kernel Wendland is used since Cubic is not available.
    case KERNEL_Wendland:    CalculeCpuT<KERNEL_Wendland>  (timestep,dvd,npbok,npb,np,pos,code,idp,velrhop,sigma);  break;
    default: Run_Exceptioon("Kernel unknown.");
  }
}
  ...
  ,const typecode *code,const unsigned *idp,const float4 *velrhop,const tsymatrix3f *sigma,float3 *aux)
{
  ...
  cugauge::Interaction_GaugeForce(CSP,dvd,n,IdBegin,Code
    ,posxy,posz,code,idp,velrhop,sigma,PartAceg);
