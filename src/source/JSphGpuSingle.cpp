...
void JSphGpuSingle::ConfigDomain(){
  ...
  for (unsigned p = Npb; p < Np; p++)
  {
      ...
      Velrhop[p].w = RhopZero;
      //  if (SolidH != 0) {
      //	  Sigma[p].zz=float(RhopZero*CSP.gravity.z*(SolidH - AuxPos[p].z));
      //  }
      //  else { Sigma[p].zz = 0; }
      //  Sigma[p].xx = float((1. - sin(SoilCte.phi))*Sigma[p].zz);
      //  Sigma[p].yy = float((1. - sin(SoilCte.phi))*Sigma[p].zz);
  }
      ...
      for(unsigned cblock=0;cblock<2;cblock++){//-0:new periodic particles, 1:original periodic particles
        ...
        while(run && num2){
          ...
          if(count>nmax || !CheckGpuParticlesSize(count+Np)){
            ...
            if(PorePressureAceDiffg)cusph::PeriodicDuplicateFloat3(count,Np,listpg,PorePressureAceDiffg);
...
void JSphGpuSingle::RunCellDivide(bool updateperiodic){
  ...
  Timersg->TmStart(TMG_NlSortData,false);
  {
    ...
    float3* porepressureacediffg = (PorePressureAceDiffg? ArraysGpu->ReserveFloat3(): NULL);
    ...
    if(PorePressureAceDiffg)CellDivSingle->SortDataArrays(PorePressureAceDiffg,porepressureacediffg);
    ...
    if(PorePressureAceDiffg){ swap(PorePressureAceDiffg,porepressureacediffg); ArraysGpu->Free(porepressureacediffg); }
    //====    
...
//<vs_cdbc_ini>
//==============================================================================
/// Calculates interpolated data on boundary particles from fluid domain for cDBC.
/// Calcula datos extrapolados en el contorno para mDBC.
//==============================================================================
void JSphGpuSingle::CdbcBoundCorrection(){
  Timersg->TmStart(TMG_CfPreForces,false);
  //unsigned n=Np;
...
void JSphGpuSingle::RunGaugeSystem(double timestep,bool saveinput){
  if(!Nstep || GaugeSystem->GetCount()){
    ...
    GaugeSystem->CalculeGpu(timestep,DivData
      ,NpbOk,Npb,Np,Posxyg,Poszg,Codeg,Idpg,Velrhopg,Sigmag,saveinput);
...
void JSphGpuSingle::SaveData(){
  ...
  if(SavePorePressure && PorePressg){
    ...
    for(unsigned p=0;p<npsave;p++){
      if(Idp[p]>=CaseNbound){
        const double z=GetHydraulicElevation(AuxPos[p]);
    ...
    if(PorePressureAceDiffg){
      tfloat3 *porepressureacediff=SaveNormalArrayGpu(npsave,PorePressureAceDiffg);
      arrays.AddArray("PorePressureAccelDiff",npsave,porepressureacediff,true);
    }
  }
