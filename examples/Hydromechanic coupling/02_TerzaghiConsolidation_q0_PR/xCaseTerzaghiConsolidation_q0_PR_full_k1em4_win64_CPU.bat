...
if exist "%dirout%" (
  if /i "%~1"=="-force" (
    ...
    choice /c YN /m "Delete it and rerun k=1e-4 full validation"
...
rem q0 Terzaghi consolidation full validation, k=1e-4 m/s.
...
:success
echo q0 Terzaghi consolidation k=1e-4 full validation done.
popd
exit /b 0

:abort
echo q0 Terzaghi consolidation k=1e-4 full validation aborted by user.
popd
exit /b 0

:fail
echo q0 Terzaghi consolidation k=1e-4 full validation aborted.
popd
exit /b 1
