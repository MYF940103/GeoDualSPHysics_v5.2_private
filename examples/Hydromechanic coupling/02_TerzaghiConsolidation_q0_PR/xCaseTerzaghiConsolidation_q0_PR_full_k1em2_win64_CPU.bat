...
set case=CaseTerzaghiConsolidation_q0_PR_full_k1em2
set dirout=%case%_out
set diroutdata=%dirout%\data
set tmax=0.364471428571429
set tout=0.00182185714285714
...
if exist "%dirout%" (
  if /i "%~1"=="-force" (
    ...
    choice /c YN /m "Delete it and rerun k=1e-2 full validation"
...
rem q0 Terzaghi consolidation full validation, k=1e-2 m/s.
...
:success
echo q0 Terzaghi consolidation k=1e-2 full validation done.
popd
exit /b 0

:abort
echo q0 Terzaghi consolidation k=1e-2 full validation aborted by user.
popd
exit /b 0

:fail
echo q0 Terzaghi consolidation k=1e-2 full validation aborted.
popd
exit /b 1
