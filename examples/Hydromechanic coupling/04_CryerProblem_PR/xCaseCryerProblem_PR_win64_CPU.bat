...
%dualsphysicscpu% -cpu "%stage1out%/%case%" "%stage1out%" -dirdataout data -svres -svextraparts:1 -tmax:%stage1_tmax% -tout:%stage1_tout%
if not "%ERRORLEVEL%" == "0" goto fail
...
%dualsphysicscpu% -cpu "%stage2out%/%case%" "%stage2out%" -dirdataout data -svres -svextraparts:1 -partbegin:%partbegin%:0 "%stage1data%" -tmax:%stage2_tmax% -tout:%stage2_tout%
if not "%ERRORLEVEL%" == "0" goto fail
