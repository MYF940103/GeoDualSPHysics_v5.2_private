...
rem Shallow-directory Cryer area-normalized SphereNormal test.
set stage1_tmax=0.082
set stage1_tout=0.002
set stage2_tmax=0.0911
...
del /q "%figdir%\area_load_center_pressure.*" 2>nul

rem Stage 1: undrained area-normalized spherical normal loading.
...
rem Stage 2: restart from the undrained state and open FreeSurface drainage.
...
set CRYER_OUTTAG=area_load_center_pressure
set CRYER_SUMMARY_JSON=%figdir%\area_load_center_pressure_summary.json
...
:success
echo Cryer area-load two-stage validation done.
...
:fail
echo Cryer area-load validation aborted.
popd
exit /b 1
