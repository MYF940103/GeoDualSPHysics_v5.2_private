...
FIGDIR = ROOT / "figures" / "formal_validation"
RHO_W = 1000.0
G_REF = 9.81
NROOTS = 420
TV_MIN_NUM = 0.001
...
GROUPS = [
    {"tag": "nu010", "case": "CaseCryerProblem_PR_dp002_rampclosed_nu010"},
    {"tag": "nu020", "case": "CaseCryerProblem_PR_dp002_rampclosed_nu020"},
    {"tag": "nu030", "case": "CaseCryerProblem_PR_dp002_rampclosed_nu030"},
    {"tag": "nu045", "case": "CaseCryerProblem_PR_dp002_rampclosed_nu045"},
]
...
def parse_xml(case_name):
    ...
    ramp_time = value("HydroMechTopLoadRampTime", 0.0)
    drainage_start = value("HydroMechDrainageStartTime", ramp_time)
    return {
...
def write_csv(path, rows):
    keys = [
        "nu", "time_s", "time_aligned_s", "tv", "center_pore_pa",
        "center_pore_over_p0", "theory_pore_over_p0", "error",
        "sample_count", "sample_radius_mean_m",
    ]
...
def read_csv(path):
    ...
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            out.append({
                key: int(float(value)) if key == "sample_count" else float(value)
...
def process_group(group, force=False):
    ...
    nu = meta["nu"]
    out_csv = FIGDIR / f"cryer_poisson_dp002_rampclosed_{group['tag']}_center_r1dp.csv"
    ...
    for pos, path in enumerate(files, start=1):
        ...
        time_aligned = max(0.0, time - meta["tv_origin_time"])
        tv = cv * time_aligned / (meta["radius"] * meta["radius"])
        if tv + 1e-12 < TV_MIN_NUM or tv > TV_MAX + 1e-9:
            continue
        stat = read_center_pore(path, meta["dp"])
        num = stat["center_pore_pa"] / meta["q0"]
        ana = theory(tv)
        rows.append({
            "nu": nu,
            "time_s": time,
            "time_aligned_s": time_aligned,
            "tv": tv,
            "center_pore_pa": stat["center_pore_pa"],
            "center_pore_over_p0": num,
            "theory_pore_over_p0": ana,
            "error": num - ana,
...
def interp_num_at_tv(rows, tv):
    xs = np.array([r["tv"] for r in rows], dtype=float)
    ys = np.array([r["center_pore_over_p0"] for r in rows], dtype=float)
    if tv <= xs[0]:
        return float(ys[0])
    if tv >= xs[-1]:
        return float(ys[-1])
    return float(np.interp(tv, xs, ys))


def dense_theory_peak(nu):
    theory = theory_function(nu)
    tvs = np.logspace(math.log10(XMIN), math.log10(TV_MAX), 5000)
    vals = np.array([theory(float(tv)) for tv in tvs])
    imax = int(np.nanargmax(vals))
    return float(vals[imax]), float(tvs[imax])
...
def metrics_for(rows):
    ...
    peak_theory, peak_theory_tv = dense_theory_peak(rows[0]["nu"])
    num_at_theory_peak = interp_num_at_tv(rows, peak_theory_tv)
    early = [r for r in rows if r["tv"] <= 0.01 + 1e-12]
    early_errors = np.array([r["error"] for r in early], dtype=float)
    return {
        "nu": rows[0]["nu"],
        "points": len(rows),
        ...
        "peak_theory_tv": peak_theory_tv,
        "num_at_peak_theory_tv": num_at_theory_peak,
        "peak_deficit_at_theory_peak": num_at_theory_peak - peak_theory,
        "early_rmse_tv_le_0p01": float(math.sqrt(float(np.mean(early_errors * early_errors)))) if early else None,
...
def plot(group_data):
    ...
    theory_color = "#3C5488"
    sph_color = "#E64B35"
    theory_dash = (0, (3.2, 2.2))
    sph_markers = ["o", "s", "^", "D"]
    fig, ax = plt.subplots(figsize=(7.4, 5.7), dpi=240)

    theory_tvs = np.logspace(math.log10(XMIN), math.log10(XMAX), 1000)
    ymax = 1.0
    annotation_specs = {
    ...
    for i, (_, meta, rows, _) in enumerate(group_data):
        nu = meta["nu"]
        theory = theory_function(nu)
        ...
        ax.plot(theory_tvs, theory_vals, color=theory_color,
                linestyle=theory_dash, linewidth=1.65)
        ax.plot(tv, num, linestyle="none", marker=sph_markers[i], markevery=6,
                markersize=3.6, markerfacecolor="none", markeredgewidth=1.0,
                color=sph_color)
    ...
    ax.grid(True, which="minor", axis="x", alpha=0.24, linestyle=":", linewidth=0.7)

    legend_handles = [
        Line2D([0], [0], color=theory_color, linestyle=theory_dash,
               linewidth=1.65, label="theoretical solution"),
...
def main():
    ...
    for group in GROUPS:
        ...
        if not rows:
            raise RuntimeError(f"No rows generated for {group['case']}")
        all_rows.extend(rows)
        group_data.append((group, meta, rows, metrics_for(rows)))

    combined_csv = FIGDIR / "cryer_poisson_sweep_dp002_rampclosed_center_r1dp.csv"
    write_csv(combined_csv, all_rows)

    metrics = [item[3] for item in group_data]
    metrics_json = FIGDIR / "cryer_poisson_sweep_dp002_rampclosed_center_r1dp_metrics.json"
    metrics_json.write_text(json.dumps(metrics, indent=2), encoding="utf-8")

    fig = plot(group_data)
    png = FIGDIR / "cryer_poisson_sweep_dp002_rampclosed_center_r1dp_paper_axes.png"
    pdf = FIGDIR / "cryer_poisson_sweep_dp002_rampclosed_center_r1dp_paper_axes.pdf"
    svg = FIGDIR / "cryer_poisson_sweep_dp002_rampclosed_center_r1dp_paper_axes.svg"
    fig.savefig(png)
    fig.savefig(pdf)
    fig.savefig(svg)
    plt.close(fig)
    ...
    for row in metrics:
        print(
            ...
            f"deficit@theory_peak={row['peak_deficit_at_theory_peak']:.6g}, "
            f"RMSE={row['rmse']:.6g}",
            flush=True,
        )
