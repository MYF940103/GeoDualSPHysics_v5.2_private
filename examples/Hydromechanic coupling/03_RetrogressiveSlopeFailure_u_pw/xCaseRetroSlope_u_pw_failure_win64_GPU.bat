...
set name=CaseRetroSlope_u_pw_failure
set dirout=%name%_GPU_out
set diroutdata=%dirout%\data
set initname=CaseRetroSlope_u_pw_prestress
...
if not exist "%initdir%\Part_%partbegin4%.bi4" (
  ...
  echo Run the updated coupled prestress stage first: xCaseRetroSlope_u_pw_prestress_win64_CPU.bat or xCaseRetroSlope_u_pw_prestress_win64_GPU.bat.
  goto fail
)
...
%dualsphysicsgpu% -gpu -mdbc_noslip "%dirout%/%name%" "%dirout%" -dirdataout data -svres -partbegin:%partbegin%:0 "%initdir%" %runextra%
if not "%ERRORLEVEL%" == "0" goto fail
...
:success
echo u-pw retrogressive failure GPU stage completed.
popd
if "%force%" == "1" exit /b 0
pause
exit /b 0
...
:fail
echo u-pw retrogressive failure GPU stage aborted.
popd
if "%force%" == "1" exit /b 1
pause
exit /b 1
