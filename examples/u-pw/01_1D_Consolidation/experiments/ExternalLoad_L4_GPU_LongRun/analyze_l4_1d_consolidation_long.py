#!/usr/bin/env python3
"""Postprocess the L4 consistent initial-state 1D consolidation gate."""
...
SUMMARY_CSV = Path("l4_case_summary.csv")
INITIAL_STATE_CSV = Path("l4_initial_state_metrics.csv")
ANALYTICAL_PROFILES_CSV = Path("l4_analytical_profiles.csv")
PROFILE_METRICS_CSV = Path("l4_profile_metrics.csv")
BOTTOM_METRICS_CSV = Path("l4_bottom_pressure_metrics.csv")
BOUNDARY_METRICS_CSV = Path("l4_boundary_metrics.csv")
STRESS_STATE_CSV = Path("l4_stress_state_metrics.csv")
COMPARISON_CSV = Path("l4_l3a_l3b_comparison.csv")
CPU_GPU_PARITY_CSV = Path("l4_cpu_gpu_parity_metrics.csv")
ERROR_OVER_TIME_CSV = Path("l4_error_over_time.csv")
TV_MAPPING_CSV = Path("l4_tv_mapping.csv")
...
def select_profile_times(times: list[float]) -> list[float]:
    targets = [
        0.0,
        DRAIN_START,
        min(TIME_MAX, max(TIME_OUT, 0.05 * TIME_MAX)),
        min(TIME_MAX, 0.25 * TIME_MAX),
        min(TIME_MAX, 0.50 * TIME_MAX),
        min(TIME_MAX, 0.75 * TIME_MAX),
        TIME_MAX,
    ]
    selected = []
...
def analyze_run(label: str, outdir: Path):
    ...
    # Fit the analytical amplitude as a diagnostic only. For this L4 gate the
    ...
    bottom_deltas = [
        float(bottom_rows[i + 1]["bottom_excess_mean"]) - float(bottom_rows[i]["bottom_excess_mean"])
        for i in range(len(bottom_rows) - 1)
    ]
    mean_deltas = [
        float(frame_rows[i + 1]["ExcessPorePress_mean"]) - float(frame_rows[i]["ExcessPorePress_mean"])
        for i in range(len(frame_rows) - 1)
    ]
    tv_final = CV * max(TIME_MAX - DRAIN_START, 0.0) / (h_eff * h_eff) if h_eff > 0 else 0.0
    summary = {
        "run": label,
        **info,
        "TimeMax_s": TIME_MAX,
        ...
        "max_positive_bottom_excess_delta_Pa": max(bottom_deltas) if bottom_deltas else 0.0,
        "max_positive_mean_excess_delta_Pa": max(mean_deltas) if mean_deltas else 0.0,
        "monotonic_decay_indicator": "pass" if (not bottom_deltas or max(bottom_deltas) <= 1.0e-6) else "check",
...
def plot_outputs(results: dict[str, dict]) -> None:
    ...
    for stem, field_sim, field_a, xlabel in [
        ("l4_excess_profiles_vs_analytical", "sim_excess", "analytical_excess_q0", "Excess pore pressure [Pa]"),
        ("l4_pore_pressure_profiles", "sim_porepress", "analytical_total_q0", "Total pore pressure [Pa]"),
    ]:
    ...
    for stem, ylabel, sim_key, a_key in [
        ("l4_bottom_excess_vs_analytical", "Bottom excess pressure [Pa]", "bottom_excess_mean", "analytical_q0_bottom"),
        ("l4_bottom_pore_pressure_vs_analytical", "Bottom total pore pressure [Pa]", "bottom_porepress_mean", "analytical_q0_total_bottom"),
    ]:
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_relative_error_time.svg")
    fig.savefig(FIGDIR / "l4_relative_error_time.png", dpi=180)
    plt.close(fig)
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_boundary_checks.svg")
    fig.savefig(FIGDIR / "l4_boundary_checks.png", dpi=180)
    plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_cpu_gpu_comparison.svg")
    fig.savefig(FIGDIR / "l4_cpu_gpu_comparison.png", dpi=180)
    plt.close(fig)

    # 8. Volume-mean excess decay.
    fig, ax = plt.subplots(figsize=(6.4, 4.0))
    for label in sorted(results):
        rs = [r for r in frames if r["run"] == label]
        ax.plot([float(r["time_s"]) for r in rs], [float(r["ExcessPorePress_mean"]) for r in rs], marker="o", markersize=2, label=label)
    ax.set_xlabel("Time [s]")
    ax.set_ylabel("Volume-mean excess pressure [Pa]")
    ax.grid(True, alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_volume_mean_excess_decay.svg")
    fig.savefig(FIGDIR / "l4_volume_mean_excess_decay.png", dpi=180)
    plt.close(fig)

    # 9. Profile RMSE over Tv.
    profile_metrics = rows("profile_metrics")
    fig, ax = plt.subplots(figsize=(6.4, 4.0))
    for label in sorted(results):
        rs = [r for r in profile_metrics if r["run"] == label]
        if not rs:
            continue
        h_eff = float(results[label]["summary"]["H_effective_m"])
        tv = [CV * max(float(r["time_s"]) - DRAIN_START, 0.0) / (h_eff * h_eff) for r in rs]
        ax.plot(tv, [float(r["rmse_q0"]) for r in rs], marker="o", markersize=3, label=f"{label} RMSE")
        ax.plot(tv, [float(r["relative_rmse_q0"]) for r in rs], marker="s", markersize=3, linestyle="--", label=f"{label} relative RMSE")
    ax.set_xlabel("Terzaghi Tv [-]")
    ax.set_ylabel("Profile error [Pa or -]")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_rmse_vs_tv.svg")
    fig.savefig(FIGDIR / "l4_rmse_vs_tv.png", dpi=180)
    plt.close(fig)

    # 10. Validation-level schematic for reports/slides.
    fig, ax = plt.subplots(figsize=(7.2, 2.4))
    ax.axis("off")
    boxes = [
        ("Level 1\nPR diffusion + hydraulic boundaries\nL3c/L4", 0.05, "#d9ead3"),
        ("Level 2\nmechanical q0 generation\nL5 deferred", 0.38, "#fff2cc"),
        ("Level 3\nfull feedback coupling\ndeferred", 0.71, "#f4cccc"),
    ]
    for text, x0, color in boxes:
        rect = plt.Rectangle((x0, 0.25), 0.27, 0.50, facecolor=color, edgecolor="0.3")
        ax.add_patch(rect)
        ax.text(x0 + 0.135, 0.50, text, ha="center", va="center", fontsize=9)
    ax.annotate("", xy=(0.38, 0.50), xytext=(0.32, 0.50), arrowprops={"arrowstyle": "->", "color": "0.3"})
    ax.annotate("", xy=(0.71, 0.50), xytext=(0.65, 0.50), arrowprops={"arrowstyle": "->", "color": "0.3"})
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_level1_vs_level2_schematic.svg")
    fig.savefig(FIGDIR / "l4_level1_vs_level2_schematic.png", dpi=180)
    plt.close(fig)

    # 11. L2 AccInput vs L4 consistent initial-state gate when L2 metrics are retained.
    ...
    if l2.exists():
        ...
        for label in sorted(results):
            rs = [r for r in bottom if r["run"] == label]
            ax.plot([float(r["time_s"]) for r in rs], [float(r["bottom_excess_mean"]) for r in rs], marker="o", markersize=2, label=f"L4 {label}")
        ax.set_xlabel("Time [s]")
        ax.set_ylabel("Bottom excess pressure [Pa]")
        ax.grid(True, alpha=0.25)
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(FIGDIR / "l4_l2_bottom_comparison.svg")
        fig.savefig(FIGDIR / "l4_l2_bottom_comparison.png", dpi=180)
        plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_velocity_max_time.svg")
    fig.savefig(FIGDIR / "l4_velocity_max_time.png", dpi=180)
    plt.close(fig)
    ...
    if initial_profiles:
        ...
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(FIGDIR / "l4_initial_excess_profile.svg")
        fig.savefig(FIGDIR / "l4_initial_excess_profile.png", dpi=180)
        plt.close(fig)
...
def comparison_rows(summary_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in summary_rows:
        rows.append({
            "case": f"L4 consistent initial state {row.get('run', '')}",
...
def cpu_gpu_parity_rows(results: dict[str, dict]) -> list[dict[str, object]]:
    if "cpu" not in results or "gpu" not in results:
        return [{
            "cpu_run_available": "false",
            "gpu_run_available": str("gpu" in results).lower(),
            "basis": "L4 matching CPU run skipped because L3c CPU runtime was about 393 s for 0.02 s; use L3c CPU/GPU parity as reference",
            "common_frame_count": 0,
            "bottom_excess_max_abs_diff_Pa": "",
            "mean_excess_max_abs_diff_Pa": "",
            "top_residual_max_abs_diff_Pa": "",
            "bottom_no_flux_max_abs_diff_Pa": "",
            "profile_rmse_q0_diff_Pa": "",
            "bottom_rmse_q0_diff_Pa": "",
        }]

    cpu_frames = {round(float(r["time_s"]), 12): r for r in results["cpu"]["frames"]}
    gpu_frames = {round(float(r["time_s"]), 12): r for r in results["gpu"]["frames"]}
    common = sorted(set(cpu_frames).intersection(gpu_frames))
    cpu_summary = results["cpu"]["summary"]
    gpu_summary = results["gpu"]["summary"]

    def diff(key: str) -> list[float]:
        return [abs(float(cpu_frames[t][key]) - float(gpu_frames[t][key])) for t in common]

    return [{
        "cpu_run_available": "true",
        "gpu_run_available": "true",
        "basis": "matching L4 CPU/GPU run",
        "common_frame_count": len(common),
        "bottom_excess_max_abs_diff_Pa": max(diff("bottom_excess_mean")) if common else 0.0,
        "mean_excess_max_abs_diff_Pa": max(diff("ExcessPorePress_mean")) if common else 0.0,
        "top_residual_max_abs_diff_Pa": max(diff("top_drained_excess_maxAbs")) if common else 0.0,
        "bottom_no_flux_max_abs_diff_Pa": max(diff("bottom_no_flux_proxy")) if common else 0.0,
        "profile_rmse_q0_diff_Pa": abs(float(cpu_summary["profile_rmse_q0_Pa"]) - float(gpu_summary["profile_rmse_q0_Pa"])),
        "bottom_rmse_q0_diff_Pa": abs(float(cpu_summary["bottom_rmse_q0_Pa"]) - float(gpu_summary["bottom_rmse_q0_Pa"])),
    }]


def error_over_time_rows(results: dict[str, dict]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for label, res in results.items():
        for row in res["bottom"]:
            rows.append({
                "run": label,
                "time_s": row["time_s"],
                "Tv": row["Tv"],
                "bottom_excess_mean": row["bottom_excess_mean"],
                "analytical_q0_bottom": row["analytical_q0_bottom"],
                "bottom_error_q0": row["error_q0"],
                "bottom_relative_error_q0": row["relative_error_q0"],
            })
    return rows


def tv_mapping_rows(results: dict[str, dict]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for label, res in results.items():
        h_eff = float(res["summary"]["H_effective_m"])
        for row in res["frames"]:
            time_s = float(row["time_s"])
            tv = CV * max(time_s - DRAIN_START, 0.0) / (h_eff * h_eff) if h_eff > 0 else 0.0
            rows.append({
                "run": label,
                "time_s": time_s,
                "Tv": tv,
                "H_effective_m": h_eff,
                "cv_m2_s": CV,
                "volume_mean_excess_Pa": row["ExcessPorePress_mean"],
                "bottom_excess_mean_Pa": row["bottom_excess_mean"],
                "top_drained_excess_maxAbs_Pa": row["top_drained_excess_maxAbs"],
                "bottom_no_flux_proxy_Pa": row["bottom_no_flux_proxy"],
            })
    return rows
...
def plot_comparison_bars(rows: list[dict[str, object]]) -> None:
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l4_l3a_l3b_l2_pressure_comparison.svg")
    fig.savefig(FIGDIR / "l4_l3a_l3b_l2_pressure_comparison.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    for label, outdir in RUNS:
        ...
        if res is not None:
            results[label] = res
    if not results:
        raise SystemExit("No CPU/GPU PartCsv outputs found for L4.")
    ...
    write_csv(CPU_GPU_PARITY_CSV, cpu_gpu_parity_rows(results))
    write_csv(ERROR_OVER_TIME_CSV, error_over_time_rows(results))
    write_csv(TV_MAPPING_CSV, tv_mapping_rows(results))
