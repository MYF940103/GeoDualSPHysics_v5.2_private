#!/usr/bin/env python3
"""Postprocess the L2 paper-aligned external-load 1D consolidation case."""
...
CASE = "Case1DConsolidation_PR_ExternalLoad_L2"
...
SUMMARY_CSV = Path("l2_case_summary.csv")
ANALYTICAL_PROFILES_CSV = Path("l2_analytical_profiles.csv")
PROFILE_METRICS_CSV = Path("l2_profile_metrics.csv")
BOTTOM_METRICS_CSV = Path("l2_bottom_pressure_metrics.csv")
BOUNDARY_METRICS_CSV = Path("l2_boundary_metrics.csv")
...
DRAIN_START = read_xml_value("PorePressureTopDrainedStartTime", 0.005)
...
def analyze_run(label: str, outdir: Path):
    ...
    # Fit the analytical amplitude to the first available post-drainage bottom
    # frame.  This separates the current reduced AccInput loading amplitude from
    # the ideal q0=10 kPa Terzaghi target.
    fit_amp = Q0_EXCESS
    ...
    summary = {
        ...
        **info,
        ...
        "AccInput_az_m_s2": -Q0_EXCESS / (RHO0 * DP),
        "fit_amplitude_Pa": fit_amp,
        ...
        "load_route": "native AccInput on mkfluid=1 top material layer",
    }
    return {
        "summary": summary,
        "frames": frame_rows,
...
def plot_outputs(results: dict[str, dict]) -> None:
    ...
    for stem, field_sim, field_a, xlabel in [
        ("l2_excess_profiles_vs_analytical", "sim_excess", "analytical_excess_q0", "Excess pore pressure [Pa]"),
        ("l2_pore_pressure_profiles", "sim_porepress", "analytical_total_q0", "Total pore pressure [Pa]"),
    ]:
    ...
    for stem, ylabel, sim_key, a_key in [
        ("l2_bottom_excess_vs_analytical", "Bottom excess pressure [Pa]", "bottom_excess_mean", "analytical_q0_bottom"),
        ("l2_bottom_pore_pressure_vs_analytical", "Bottom total pore pressure [Pa]", "bottom_porepress_mean", "analytical_q0_total_bottom"),
    ]:
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l2_relative_error_time.svg")
    fig.savefig(FIGDIR / "l2_relative_error_time.png", dpi=180)
    plt.close(fig)
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGDIR / "l2_boundary_checks.svg")
    fig.savefig(FIGDIR / "l2_boundary_checks.png", dpi=180)
    plt.close(fig)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIGDIR / "l2_cpu_gpu_comparison.svg")
    fig.savefig(FIGDIR / "l2_cpu_gpu_comparison.png", dpi=180)
    plt.close(fig)

    # 8. L1 vs L2 if retained L1 metrics are present.
    l1 = Path("../ExternalLoad_L1_Baseline/l1_external_load_frame_metrics.csv")
    if l1.exists():
        with l1.open(newline="") as fp:
            l1rows = list(csv.DictReader(fp))
        fig, ax = plt.subplots(figsize=(6.4, 4.0))
        ax.plot([float(r["time_s"]) for r in l1rows], [float(r["bottom_excess_mean"]) for r in l1rows], label="L1 GPU smoke", linewidth=1.5)
        for label in sorted(results):
            rs = [r for r in bottom if r["run"] == label]
            ax.plot([float(r["time_s"]) for r in rs], [float(r["bottom_excess_mean"]) for r in rs], marker="o", markersize=2, label=f"L2 {label}")
        ax.set_xlabel("Time [s]")
        ax.set_ylabel("Bottom excess pressure [Pa]")
        ax.grid(True, alpha=0.25)
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(FIGDIR / "l2_l1_l2_bottom_comparison.svg")
        fig.savefig(FIGDIR / "l2_l1_l2_bottom_comparison.png", dpi=180)
        plt.close(fig)

    # Velocity / rate diagnostics are useful when L2 is dynamically stiff.
    fig, ax = plt.subplots(figsize=(6.4, 4.0))
    for label in sorted(results):
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIGDIR / "l2_velocity_max_time.svg")
    fig.savefig(FIGDIR / "l2_velocity_max_time.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    for label, outdir in RUNS:
        ...
        if res is not None:
            results[label] = res
    if not results:
        raise SystemExit("No CPU/GPU PartCsv outputs found for L2.")
