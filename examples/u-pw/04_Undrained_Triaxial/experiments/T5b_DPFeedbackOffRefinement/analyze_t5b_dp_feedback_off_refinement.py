#!/usr/bin/env python3
"""Postprocess T5b platen reaction diagnostic cases."""
...
CASES = {
    "elastic": "CaseT5b_Elastic_ReactionRefinement",
    "dp_high_strength": "CaseT5b_DPHighStrength_ReactionRefinement",
    "dp_mild_yield": "CaseT5b_DPMildYield_ReactionRefinement",
}
...
def frame_metrics(case_key: str, case: str) -> tuple[list[dict[str, float | int | str]], list[dict[str, float | int | str]]]:
    ...
    for pf in files:
        ...
        frame = {
            ...
            "kplastic_mean": safe_mean(float(r.get("Kplastic", 0.0)) for r in spec),
            "kplastic_nonzero_count": sum(1 for r in spec if abs(float(r.get("Kplastic", 0.0))) > 1e-12),
            "true_reaction_available": 0,
            "reaction_type": "specimen_stress_proxy",
            **full_stress,
        }
...
def plot_pq(rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t5b_pq_proxy.{ext}", dpi=180)
    plt.close()
...
def plot_measurement_sensitivity(region_rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t5b_measurement_region_sensitivity.{ext}", dpi=180)
    plt.close()
...
def main() -> None:
    ...
    write_csv(ROOT / "t5b_case_summary.csv", summaries)
    write_csv(ROOT / "t5b_platen_motion_metrics.csv", all_frames)
    write_csv(ROOT / "t5b_dp_plasticity_metrics.csv", all_frames)
    write_csv(ROOT / "t5b_stress_path_metrics.csv", all_frames)
    write_csv(ROOT / "t5b_axial_stress_strain_metrics.csv", reaction_strain)
    write_csv(ROOT / "t5b_pore_pressure_strain_metrics.csv", all_frames)
    write_csv(ROOT / "t5b_pore_pressure_metrics.csv", all_frames)
    write_csv(ROOT / "t5b_measurement_region_metrics.csv", all_regions)
    write_csv(ROOT / "t5b_confinement_diagnostics.csv", all_confinement)
    write_csv(ROOT / "t5b_true_reaction_metrics.csv", all_reactions)
    write_csv(ROOT / "t5b_platen_reaction_metrics.csv", all_reactions)
    write_csv(ROOT / "t5b_reaction_vs_proxy_comparison.csv", comparison)
    write_csv(ROOT / "t5b_axial_stress_metrics.csv", comparison)
    write_csv(ROOT / "t5b_group_counts.csv", group_counts)

    if all_frames:
        plot_series(all_frames, "top_platen_displacement", "t5b_top_platen_displacement", "top platen displacement [m]")
        plot_series(all_frames, "specimen_axial_strain_proxy", "t5b_axial_strain_proxy", "specimen axial strain proxy")
        plot_series(all_frames, "reaction_force_proxy", "t5b_reaction_force_proxy", "reaction force proxy [N]")
        plot_series(all_frames, "sigma_a_proxy", "t5b_axial_stress_proxy", "axial stress proxy [Pa]")
        plot_pq(all_frames)
        plot_series(all_frames, "q_proxy", "t5b_q_vs_time", "q proxy [Pa]")
        plot_series(all_frames, "q_proxy", "t5b_q_vs_axial_strain", "q proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "p_eff_proxy", "t5b_p_eff_vs_axial_strain", "p' proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "porepress_mean", "t5b_pore_pressure_time", "specimen PorePress mean [Pa]")
        plot_series(all_frames, "porepress_mean", "t5b_pore_pressure_strain", "specimen PorePress mean [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "divvel_maxabs", "t5b_divvel_maxabs", "DivVel maxAbs [1/s]")
        plot_series(all_frames, "kplastic_max", "t5b_kplastic_max", "Kplastic max")
        plot_series(all_frames, "kplastic_nonzero_count", "t5b_kplastic_nonzero_count", "Kplastic nonzero particle count")
        plot_series(all_frames, "velocity_max", "t5b_velocity_max", "velocity max [m/s]")
        plot_series(all_frames, "porepressrate_maxabs", "t5b_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    if all_regions:
        plot_measurement_sensitivity(all_regions)
    if all_confinement:
        plot_series(all_confinement, "lateral_inward_radial_accel_mean", "t5b_lateral_confinement_accel", "lateral inward accel mean [m/s2]")
    if all_reactions:
        plot_series(all_reactions, "top_force_z", "t5b_top_reaction_fz", "top reaction Fz [N]")
        plot_series(all_reactions, "bottom_force_z", "t5b_bottom_reaction_fz", "bottom reaction Fz [N]")
        plot_series(all_reactions, "top_axial_stress", "t5b_top_reaction_axial_stress", "top reaction axial stress [Pa]")
        plot_series(all_reactions, "bottom_axial_stress", "t5b_bottom_reaction_axial_stress", "bottom reaction axial stress [Pa]")
        plot_series(all_reactions, "force_balance_error", "t5b_force_balance_error", "force balance error")
    if reaction_strain:
        plot_series(reaction_strain, "reaction_force_avg_compression", "t5b_pairwise_reaction_vs_strain", "pairwise reaction avg [N]", xkey="specimen_axial_strain_proxy")
        plot_series(reaction_strain, "reaction_axial_stress_avg", "t5b_reaction_axial_stress_vs_strain", "reaction axial stress avg [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(reaction_strain, "reaction_over_proxy_force", "t5b_reaction_proxy_ratio", "pairwise reaction / Fz_proxy")
