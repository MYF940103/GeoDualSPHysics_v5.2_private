#!/usr/bin/env python3
"""Generate T5b DP feedback-off platen refinement XML/BAT cases."""
...
CASES = [
    {
        "name": "CaseT5b_Elastic_ReactionRefinement",
        "date": "T5b elastic feedback-off platen reaction refinement",
        "label": "elastic",
        "model": "0",
    ...
    {
        "name": "CaseT5b_DPHighStrength_ReactionRefinement",
        "date": "T5b DP high-strength feedback-off platen reaction refinement",
    ...
    {
        "name": "CaseT5b_DPMildYield_ReactionRefinement",
        "date": "T5b DP mild-yield feedback-off platen reaction refinement",
        "label": "dp_mild_yield",
        "model": "1",
...
"""
    return """      <parameter key="FlexibleConfiningStress" value="1" />
      ...
      <parameter key="PlatenReactionInterval" value="10" />
"""
...
if not "%ERRORLEVEL%" == "0" exit /b 1
echo T5b CPU Release completed for %name%.
exit /b 0
"""
...
def main() -> None:
    ...
    with (ROOT / "README.md").open("w", newline="\n") as f:
        f.write(
            "# T5b DP Feedback-Off Platen Refinement\n\n"
            "Run `py -3 make_t5b_cases.py`, then execute the generated CPU Release BAT files.\n"
            "Use `py -3 analyze_t5b_dp_feedback_off_refinement.py` after the runs.\n"
        )
