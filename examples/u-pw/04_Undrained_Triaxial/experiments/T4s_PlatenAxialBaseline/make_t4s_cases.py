#!/usr/bin/env python3
"""Generate T4s feedback-off platen axial baseline XML/BAT cases."""
...
CASES = [
    {
        "name": "CaseT4s_PlatenAxial_NoConfinement",
        "date": "T4s feedback-off top prescribed-velocity platen baseline, no lateral confinement",
        "lateral": False,
    },
    {
        "name": "CaseT4s_PlatenAxial_LateralConfinement",
        "date": "T4s feedback-off top prescribed-velocity platen baseline with lateral flexible confinement",
        "lateral": True,
    },
]
...
{confinement_params(bool(case["lateral"]))}    </parameters>
  </execution>
</case>
"""
...
if not "%ERRORLEVEL%" == "0" exit /b 1
echo T4s CPU Release completed for %name%.
exit /b 0
"""
...
def main() -> None:
    ...
    with (ROOT / "README.md").open("w", newline="\n") as f:
        f.write(
            "# T4s Platen Axial Baseline\n\n"
            "Run `py -3 make_t4s_cases.py`, then execute the generated CPU Release BAT files.\n"
            "Use `py -3 analyze_t4s_platen_axial_baseline.py` after the runs.\n"
        )
