#!/usr/bin/env python3
"""Postprocess T4s feedback-off platen axial baseline diagnostics."""
...
CASES = {
    "platen_no_confinement": "CaseT4s_PlatenAxial_NoConfinement",
    "platen_lateral_confinement": "CaseT4s_PlatenAxial_LateralConfinement",
}
...
def plot_pq(rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t4s_pq_proxy.{ext}", dpi=180)
    plt.close()
...
def plot_measurement_sensitivity(region_rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t4s_measurement_region_sensitivity.{ext}", dpi=180)
    plt.close()
...
def main() -> None:
    ...
    write_csv(ROOT / "t4s_case_summary.csv", summaries)
    write_csv(ROOT / "t4s_platen_motion_metrics.csv", all_frames)
    write_csv(ROOT / "t4s_specimen_stress_proxy.csv", all_frames)
    write_csv(ROOT / "t4s_pore_pressure_strain_metrics.csv", all_frames)
    write_csv(ROOT / "t4s_measurement_region_metrics.csv", all_regions)
    write_csv(ROOT / "t4s_confinement_metrics.csv", all_confinement)
    write_csv(ROOT / "t4s_reaction_proxy_metrics.csv", all_frames)
    write_csv(ROOT / "t4s_group_counts.csv", group_counts)

    if all_frames:
        plot_series(all_frames, "top_platen_displacement", "t4s_top_platen_displacement", "top platen displacement [m]")
        plot_series(all_frames, "specimen_axial_strain_proxy", "t4s_axial_strain_proxy", "specimen axial strain proxy")
        plot_series(all_frames, "reaction_force_proxy", "t4s_reaction_force_proxy", "reaction force proxy [N]")
        plot_series(all_frames, "sigma_a_proxy", "t4s_axial_stress_proxy", "axial stress proxy [Pa]")
        plot_pq(all_frames)
        plot_series(all_frames, "q_proxy", "t4s_q_vs_time", "q proxy [Pa]")
        plot_series(all_frames, "q_proxy", "t4s_q_vs_axial_strain", "q proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "p_eff_proxy", "t4s_p_eff_vs_axial_strain", "p' proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "porepress_mean", "t4s_pore_pressure_time", "specimen PorePress mean [Pa]")
        plot_series(all_frames, "porepress_mean", "t4s_pore_pressure_strain", "specimen PorePress mean [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "divvel_maxabs", "t4s_divvel_maxabs", "DivVel maxAbs [1/s]")
        plot_series(all_frames, "kplastic_max", "t4s_kplastic_max", "Kplastic max")
        plot_series(all_frames, "velocity_max", "t4s_velocity_max", "velocity max [m/s]")
        plot_series(all_frames, "porepressrate_maxabs", "t4s_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    if all_regions:
        plot_measurement_sensitivity(all_regions)
    if all_confinement:
        plot_series(all_confinement, "lateral_inward_radial_accel_mean", "t4s_lateral_confinement_accel", "lateral inward accel mean [m/s2]")
