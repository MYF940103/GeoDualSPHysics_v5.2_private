#!/usr/bin/env python3
"""Generate M3c CPU MCC stress-update smoke XML/BAT cases."""
...
CASES = {
    "CaseM3c_MCCHighPc_ElasticLike": {
        "description": "M3c MCC high-pc elastic-like CPU stress-update smoke",
        "pc0": "100000",
    },
    "CaseM3c_MCCMildYield": {
        "description": "M3c MCC mild-yield CPU stress-update smoke",
        "pc0": "120",
    },
}
...
MCC_PARAMS = {
    ...
    "MccStressUpdateEnabled": ("1", "Enable M3c MCC CPU stress update"),
}
...
def write_bat(case_name: str) -> None:
    ...
    bat.write_text(
        ...
        "if not \"%ERRORLEVEL%\" == \"0\" exit /b 1\n"
        f"echo M3c CPU Release completed for {case_name}.\n"
        "exit /b 0\n",
        encoding="utf-8",
    )
...
def main() -> None:
    for case_name, cfg in CASES.items():
        ...
        set_parameter(params, "TimeMax", "0.006")
