#!/usr/bin/env python3
"""T4g pore-pressure feedback formulation audit."""
...
CASES = [
    {"case": "CaseT4g_ConfOnly_FeedbackOffRef", "label": "feedback off", "operator": -1, "mode": "off", "class_filter": "off"},
    {"case": "CaseT4g_ConfOnly_Op1Unfiltered", "label": "op1 unfiltered", "operator": 1, "mode": "excess", "class_filter": "off"},
    {"case": "CaseT4g_ConfOnly_Op0Unfiltered", "label": "op0 unfiltered", "operator": 0, "mode": "excess", "class_filter": "off"},
    {"case": "CaseT4g_ConfOnly_Op1InteriorOnly", "label": "op1 interior", "operator": 1, "mode": "excess", "class_filter": "interior"},
    {"case": "CaseT4g_ConfOnly_Op0InteriorOnly", "label": "op0 interior", "operator": 0, "mode": "excess", "class_filter": "interior"},
    {"case": "CaseT4g_ConfOnly_Op1TotalInteriorOnly", "label": "op1 total interior", "operator": 1, "mode": "total", "class_filter": "interior"},
]
...
def manufactured_tests():
    rows = read_partcsv(ROOT / "CaseT4g_ConfOnly_FeedbackOffRef_out" / "data" / "PartCsv_0000.csv")
    ...
    write_table(ROOT / "t4g_feedback_manufactured_metrics.csv", out)
    return out
...
def main():
    ...
    write_table(ROOT / "t4g_feedback_variant_case_summary.csv", statuses)
    write_table(ROOT / "t4g_feedback_acceleration_by_class.csv", all_fb)
    write_table(ROOT / "t4g_reversal_metrics.csv", [{"case": s["case"], "label": s["label"], "center_reversal_time": s["center_reversal_time"], "negative_pressure_count_final": s["negative_pressure_count_final"], "final_porepress_min": s["final_porepress_min"]} for s in statuses])
    write_table(ROOT / "t4g_porepressrate_metrics.csv", [{"case": s["case"], "label": s["label"], "max_porepressrate": s["max_porepressrate"], "max_velocity": s["max_velocity"], "dtmin_adjusted": s["dtmin_adjusted"], "excluded": s["excluded"]} for s in statuses])
    write_table(ROOT / "t4g_pressure_variable_comparison.csv", [s for s in statuses if "InteriorOnly" in s["case"] and s["operator"] == 1])
    write_table(ROOT / "t4g_operator_comparison.csv", [s for s in statuses if s["operator"] in (0, 1)])
    write_table(ROOT / "t4g_confinement_diagnostics.csv", all_conf)
    write_table(ROOT / "t4g_measurement_region_metrics.csv", all_regions)

    plot_lines(all_frames, "porepressrate_maxabs", "PorePressRate maxAbs", "t4g_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    plot_lines(all_frames, "velocity_max", "Velocity max", "t4g_velocity_max", "velocity max [m/s]")
    plot_lines(all_regions, "porepress_mean", "Center-core pore pressure", "t4g_center_porepress", "PorePress [Pa]", "center_core")
    plot_lines(all_regions, "porepressrate_maxabs", "Center-core PorePressRate", "t4g_center_porepressrate", "PorePressRate [Pa/s]", "center_core")
    plot_lines(all_fb, "used_max", "Feedback acceleration used max", "t4g_feedback_used_max", "acceleration [m/s2]")
    plot_lines(all_fb, "interior_used_max", "Feedback acceleration by class: interior", "t4g_feedback_interior_class", "acceleration [m/s2]")
    plot_lines(all_fb, "lateral_used_max", "Feedback acceleration by class: lateral", "t4g_feedback_lateral_class", "acceleration [m/s2]")
    plot_lines(all_conf, "cap_axial_max", "Cap leakage diagnostic", "t4g_cap_leakage", "cap axial accel [m/s2]")
    plot_lines(all_conf, "lat_accel_mean", "Lateral confinement acceleration", "t4g_lateral_acceleration", "inward radial accel [m/s2]")
    ...
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(FIG / f"t4g_variant_porepressrate_bar.{ext}")
    plt.close()
    ...
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(FIG / f"t4g_manufactured_feedback_accel.{ext}")
    plt.close()
