#!/usr/bin/env python3
"""Generate T5 DP feedback-off platen triaxial baseline XML/BAT cases."""
...
CASES = [
    {
        "name": "CaseT5_DPHighStrength_PlatenLateralConfinement",
        "date": "T5 DP feedback-off high-strength platen baseline with lateral flexible confinement",
    ...
    {
        "name": "CaseT5_DPMildYield_PlatenLateralConfinement",
        "date": "T5 DP feedback-off mild-yield diagnostic with lateral flexible confinement",
        "label": "dp_mild_yield",
        "phi": "30",
        "coh": "50",
        "dlt": "0",
...
<case app="GenCase v5.0.197 (18-07-2020)" date="{case['date']}">
  ...
  <execution>
    <special>
      <soils>
        <SoilConstitutiveModel value="1" comment="Drucker-Prager elastoplastic skeleton, no softening" />
...
if not "%ERRORLEVEL%" == "0" exit /b 1
echo T5 CPU Release completed for %name%.
exit /b 0
"""
...
def main() -> None:
    ...
    with (ROOT / "README.md").open("w", newline="\n") as f:
        f.write(
            ...
            "Run `py -3 make_t5_cases.py`, then execute the generated CPU Release BAT files.\n"
            "Use `py -3 analyze_t5_dp_feedback_off_triaxial.py` after the runs.\n"
        )
