#!/usr/bin/env python3
"""Postprocess T5 DP feedback-off platen triaxial baseline diagnostics."""
...
CASES = {
    "dp_high_strength": "CaseT5_DPHighStrength_PlatenLateralConfinement",
    "dp_mild_yield": "CaseT5_DPMildYield_PlatenLateralConfinement",
}
...
def plot_pq(rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t5_pq_proxy.{ext}", dpi=180)
    plt.close()
...
def plot_measurement_sensitivity(region_rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t5_measurement_region_sensitivity.{ext}", dpi=180)
    plt.close()
...
def main() -> None:
    ...
    write_csv(ROOT / "t5_case_summary.csv", summaries)
    write_csv(ROOT / "t5_platen_motion_metrics.csv", all_frames)
    write_csv(ROOT / "t5_dp_plasticity_metrics.csv", all_frames)
    write_csv(ROOT / "t5_specimen_stress_path_proxy.csv", all_frames)
    write_csv(ROOT / "t5_pore_pressure_strain_metrics.csv", all_frames)
    write_csv(ROOT / "t5_measurement_region_metrics.csv", all_regions)
    write_csv(ROOT / "t5_confinement_diagnostics.csv", all_confinement)
    write_csv(ROOT / "t5_reaction_proxy_metrics.csv", all_frames)
    write_csv(ROOT / "t5_group_counts.csv", group_counts)

    if all_frames:
        plot_series(all_frames, "top_platen_displacement", "t5_top_platen_displacement", "top platen displacement [m]")
        plot_series(all_frames, "specimen_axial_strain_proxy", "t5_axial_strain_proxy", "specimen axial strain proxy")
        plot_series(all_frames, "reaction_force_proxy", "t5_reaction_force_proxy", "reaction force proxy [N]")
        plot_series(all_frames, "sigma_a_proxy", "t5_axial_stress_proxy", "axial stress proxy [Pa]")
        plot_pq(all_frames)
        plot_series(all_frames, "q_proxy", "t5_q_vs_time", "q proxy [Pa]")
        plot_series(all_frames, "q_proxy", "t5_q_vs_axial_strain", "q proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "p_eff_proxy", "t5_p_eff_vs_axial_strain", "p' proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "porepress_mean", "t5_pore_pressure_time", "specimen PorePress mean [Pa]")
        plot_series(all_frames, "porepress_mean", "t5_pore_pressure_strain", "specimen PorePress mean [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "divvel_maxabs", "t5_divvel_maxabs", "DivVel maxAbs [1/s]")
        plot_series(all_frames, "kplastic_max", "t5_kplastic_max", "Kplastic max")
        plot_series(all_frames, "kplastic_nonzero_count", "t5_kplastic_nonzero_count", "Kplastic nonzero particle count")
        plot_series(all_frames, "velocity_max", "t5_velocity_max", "velocity max [m/s]")
        plot_series(all_frames, "porepressrate_maxabs", "t5_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    if all_regions:
        plot_measurement_sensitivity(all_regions)
    if all_confinement:
        plot_series(all_confinement, "lateral_inward_radial_accel_mean", "t5_lateral_confinement_accel", "lateral inward accel mean [m/s2]")
