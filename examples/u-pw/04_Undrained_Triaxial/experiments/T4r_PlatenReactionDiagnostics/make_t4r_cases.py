#!/usr/bin/env python3
"""Generate T4r platen reaction/stress diagnostic XML/BAT cases."""
...
CASES = [
    {
        "name": "CaseT4r_PlatenAxial_NoConfinement",
        "date": "T4r top prescribed-velocity platen, no lateral confinement",
        "lateral": False,
    },
    {
        "name": "CaseT4r_PlatenAxial_LateralConfinement",
        "date": "T4r top prescribed-velocity platen with lateral flexible confinement",
        "lateral": True,
    },
]
...
if not "%ERRORLEVEL%" == "0" exit /b 1
echo T4r CPU Release completed for %name%.
exit /b 0
"""
...
def main() -> None:
    ...
    with (ROOT / "README.md").open("w", newline="\n") as f:
        f.write(
            ...
            "Run `py -3 make_t4r_cases.py`, then execute the generated CPU Release BAT files.\n"
            "Use `py -3 analyze_t4r_platen_reaction.py` after the runs.\n"
        )
