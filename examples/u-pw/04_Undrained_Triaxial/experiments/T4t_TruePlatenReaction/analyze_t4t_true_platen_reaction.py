#!/usr/bin/env python3
"""Postprocess T4t platen reaction diagnostic cases."""
...
CASES = {
    "elastic": "CaseT4t_Elastic_PlatenReaction",
    "dp_high_strength": "CaseT4t_DPHighStrength_PlatenReaction",
    "dp_mild_yield": "CaseT4t_DPMildYield_PlatenReaction",
}
...
def plot_pq(rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t4t_pq_proxy.{ext}", dpi=180)
    plt.close()
...
def plot_measurement_sensitivity(region_rows: list[dict[str, float | int | str]]) -> None:
    ...
    plt.tight_layout()
    for ext in ("svg", "png"):
        plt.savefig(FIGDIR / f"t4t_measurement_region_sensitivity.{ext}", dpi=180)
    plt.close()
...
def main() -> None:
    ...
    write_csv(ROOT / "t4t_case_summary.csv", summaries)
    write_csv(ROOT / "t4t_platen_motion_metrics.csv", all_frames)
    write_csv(ROOT / "t4t_dp_plasticity_metrics.csv", all_frames)
    write_csv(ROOT / "t4t_stress_path_metrics.csv", all_frames)
    write_csv(ROOT / "t4t_pore_pressure_strain_metrics.csv", all_frames)
    write_csv(ROOT / "t4t_measurement_region_metrics.csv", all_regions)
    write_csv(ROOT / "t4t_confinement_diagnostics.csv", all_confinement)
    write_csv(ROOT / "t4t_true_reaction_metrics.csv", all_reactions)
    write_csv(ROOT / "t4t_reaction_vs_proxy_comparison.csv", comparison)
    write_csv(ROOT / "t4t_axial_stress_metrics.csv", comparison)
    write_csv(ROOT / "t4t_group_counts.csv", group_counts)

    if all_frames:
        plot_series(all_frames, "top_platen_displacement", "t4t_top_platen_displacement", "top platen displacement [m]")
        plot_series(all_frames, "specimen_axial_strain_proxy", "t4t_axial_strain_proxy", "specimen axial strain proxy")
        plot_series(all_frames, "reaction_force_proxy", "t4t_reaction_force_proxy", "reaction force proxy [N]")
        plot_series(all_frames, "sigma_a_proxy", "t4t_axial_stress_proxy", "axial stress proxy [Pa]")
        plot_pq(all_frames)
        plot_series(all_frames, "q_proxy", "t4t_q_vs_time", "q proxy [Pa]")
        plot_series(all_frames, "q_proxy", "t4t_q_vs_axial_strain", "q proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "p_eff_proxy", "t4t_p_eff_vs_axial_strain", "p' proxy [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "porepress_mean", "t4t_pore_pressure_time", "specimen PorePress mean [Pa]")
        plot_series(all_frames, "porepress_mean", "t4t_pore_pressure_strain", "specimen PorePress mean [Pa]", xkey="specimen_axial_strain_proxy")
        plot_series(all_frames, "divvel_maxabs", "t4t_divvel_maxabs", "DivVel maxAbs [1/s]")
        plot_series(all_frames, "kplastic_max", "t4t_kplastic_max", "Kplastic max")
        plot_series(all_frames, "kplastic_nonzero_count", "t4t_kplastic_nonzero_count", "Kplastic nonzero particle count")
        plot_series(all_frames, "velocity_max", "t4t_velocity_max", "velocity max [m/s]")
        plot_series(all_frames, "porepressrate_maxabs", "t4t_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    if all_regions:
        plot_measurement_sensitivity(all_regions)
    if all_confinement:
        plot_series(all_confinement, "lateral_inward_radial_accel_mean", "t4t_lateral_confinement_accel", "lateral inward accel mean [m/s2]")
    if all_reactions:
        plot_series(all_reactions, "top_force_z", "t4t_top_reaction_fz", "top reaction Fz [N]")
        plot_series(all_reactions, "bottom_force_z", "t4t_bottom_reaction_fz", "bottom reaction Fz [N]")
        plot_series(all_reactions, "top_axial_stress", "t4t_top_reaction_axial_stress", "top reaction axial stress [Pa]")
        plot_series(all_reactions, "bottom_axial_stress", "t4t_bottom_reaction_axial_stress", "bottom reaction axial stress [Pa]")
        plot_series(all_reactions, "force_balance_error", "t4t_force_balance_error", "force balance error")
