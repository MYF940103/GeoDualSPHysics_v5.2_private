#!/usr/bin/env python3
"""Generate T4t platen reaction diagnostic XML/BAT cases."""
...
CASES = [
    {
        "name": "CaseT4t_Elastic_PlatenReaction",
        "date": "T4t elastic platen reaction diagnostic with lateral flexible confinement",
        "label": "elastic",
        "model": "0",
        "model_comment": "Linear elastic skeleton",
        "phi": "33",
        "coh": "10000",
        "dlt": "0",
    },
    {
        "name": "CaseT4t_DPHighStrength_PlatenReaction",
        "date": "T4t DP high-strength platen reaction diagnostic with lateral flexible confinement",
        "label": "dp_high_strength",
        "model": "1",
        "model_comment": "Drucker-Prager high-strength elastic-like smoke",
        "phi": "33",
        "coh": "10000",
        "dlt": "0",
    },
    {
        "name": "CaseT4t_DPMildYield_PlatenReaction",
        "date": "T4t DP mild-yield platen reaction diagnostic with lateral flexible confinement",
        "label": "dp_mild_yield",
        "model": "1",
        "model_comment": "Drucker-Prager mild-yield diagnostic",
        "phi": "30",
        "coh": "50",
        "dlt": "0",
    },
...
"""
    return """      <parameter key="FlexibleConfiningStress" value="1" />
      ...
      <parameter key="PlatenReactionInterval" value="50" />
"""
...
<case app="GenCase v5.0.197 (18-07-2020)" date="{case['date']}">
  <casedef>
    ...
    <motion>
      <objreal ref="1">
        <begin mov="1" start="0" finish="{TIME_MAX}" />
        <mvrect id="1" duration="{TIME_MAX}">
          <vel x="0" y="0" z="{TOP_VZ}" units_comment="m/s" />
        </mvrect>
      </objreal>
    </motion>
  </casedef>
  <execution>
    <special>
      <soils>
        <SoilConstitutiveModel value="{case['model']}" comment="{case['model_comment']}" />
        <phi value="{case['phi']}" comment="DP friction angle [deg]" />
        <coh value="{case['coh']}" comment="DP cohesion [Pa]" />
        <dlt value="{case['dlt']}" comment="DP dilatancy angle [deg]" />
...
{confinement_params(True)}    </parameters>
  </execution>
</case>
"""
...
if not "%ERRORLEVEL%" == "0" exit /b 1
echo T4t CPU Release completed for %name%.
exit /b 0
"""
...
def main() -> None:
    ...
    with (ROOT / "README.md").open("w", newline="\n") as f:
        f.write(
            "# T4t True Platen Reaction Diagnostics\n\n"
            "Run `py -3 make_t4t_cases.py`, then execute the generated CPU Release BAT files.\n"
            "Use `py -3 analyze_t4t_true_platen_reaction.py` after the runs.\n"
        )
