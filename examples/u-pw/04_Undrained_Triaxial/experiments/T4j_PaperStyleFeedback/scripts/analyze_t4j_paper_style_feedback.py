#!/usr/bin/env python3
"""T4j paper-style pore-pressure momentum feedback audit."""
...
CASES = [
    {"case": "CaseT4j_ConfOnly_Op1InteriorBaseline", "label": "op1 interior", "operator": 1, "mode": "excess", "class_filter": "interior", "stabilized": "off"},
    {"case": "CaseT4j_ConfOnly_Op3Unfiltered", "label": "op3 unfiltered", "operator": 3, "mode": "excess", "class_filter": "off", "stabilized": "off"},
    {"case": "CaseT4j_ConfOnly_Op3Interior", "label": "op3 interior", "operator": 3, "mode": "excess", "class_filter": "interior", "stabilized": "off"},
]
...
def manufactured_tests():
    rows = read_partcsv(ROOT / "CaseT4j_ConfOnly_Op1InteriorBaseline_out" / "data" / "PartCsv_0000.csv")
    ...
    write_table(ROOT / "t4j_manufactured_metrics.csv", out)
    return out
...
def main():
    ...
    write_table(ROOT / "t4j_case_summary.csv", statuses)
    write_table(ROOT / "t4j_pressure_acceleration_metrics.csv", all_fb)
    write_table(ROOT / "t4j_lsq_condition_metrics.csv", lsq_rows)
    write_table(ROOT / "t4j_reversal_metrics.csv", [{"case": s["case"], "label": s["label"], "center_reversal_time": s["center_reversal_time"], "negative_pressure_count_final": s["negative_pressure_count_final"], "final_porepress_min": s["final_porepress_min"]} for s in statuses])
    write_table(ROOT / "t4j_porepressrate_metrics.csv", [{"case": s["case"], "label": s["label"], "max_porepressrate": s["max_porepressrate"], "max_velocity": s["max_velocity"], "dtmin_adjusted": s["dtmin_adjusted"], "excluded": s["excluded"]} for s in statuses])
    write_table(ROOT / "t4j_operator_comparison_metrics.csv", statuses)
    write_table(ROOT / "t4j_confinement_diagnostics.csv", all_conf)
    write_table(ROOT / "t4j_measurement_region_metrics.csv", all_regions)

    plot_lines(all_frames, "porepressrate_maxabs", "PorePressRate maxAbs", "t4j_porepressrate_maxabs", "PorePressRate maxAbs [Pa/s]")
    plot_lines(all_frames, "velocity_max", "Velocity max", "t4j_velocity_max", "velocity max [m/s]")
    plot_lines(all_regions, "porepress_mean", "Center-core pore pressure", "t4j_center_porepress", "PorePress [Pa]", "center_core")
    plot_lines(all_regions, "porepressrate_maxabs", "Center-core PorePressRate", "t4j_center_porepressrate", "PorePressRate [Pa/s]", "center_core")
    plot_lines(all_fb, "used_max", "Feedback acceleration used max", "t4j_feedback_used_max", "acceleration [m/s2]")
    plot_lines(all_fb, "interior_used_max", "Feedback acceleration by class: interior", "t4j_feedback_interior_class", "acceleration [m/s2]")
    plot_lines(all_conf, "cap_axial_max", "Cap leakage diagnostic", "t4j_cap_leakage", "cap axial accel [m/s2]")
    plot_lines(all_conf, "lat_accel_mean", "Lateral confinement acceleration", "t4j_lateral_acceleration", "inward radial accel [m/s2]")
    plot_lines(all_fb, "lsq_cond_max", "LSQ condition proxy max", "t4j_lsq_condition", "condition proxy")
    ...
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(FIG / f"t4j_variant_porepressrate_bar.{ext}")
    plt.close()
    ...
    plt.title("Manufactured pressure momentum acceleration")
    plt.tight_layout()
    for ext in ("png", "svg"):
        plt.savefig(FIG / f"t4j_manufactured_feedback_accel.{ext}")
    plt.close()
