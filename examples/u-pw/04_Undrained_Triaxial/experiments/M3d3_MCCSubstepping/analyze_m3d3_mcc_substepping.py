#!/usr/bin/env python3
"""Postprocess M3d3 MCC substepping robustness diagnostics."""
...
CASES = {
    "baseline": "CaseM3d3_MCCMildBaseline",
    "fixed_substeps4": "CaseM3d3_MCCMildFixedSubsteps4",
    "adaptive_substeps16": "CaseM3d3_MCCMildAdaptiveSubsteps16",
    "adaptive_fallback16": "CaseM3d3_MCCMildAdaptiveFallback16",
    "half_speed_adaptive": "CaseM3d3_MCCMildHalfSpeedAdaptive",
}
...
def process_case(case_key: str, case: str) -> tuple[
    ...
    for path in part_files(case):
        ...
        frame = {
            ...
            "MccSubstepCount_mean": safe_mean(float(r.get("MccSubstepCount", 1.0)) for r in spec),
            "MccSubstepCount_max": max(float(r.get("MccSubstepCount", 1.0)) for r in spec),
            ...
            "MccFallbackUsed_count": sum(1 for r in spec if float(r.get("MccFallbackUsed", 0.0)) > 0.5),
            ...
            failed.append(
                {
                    ...
                    "MccSubstepCount": float(r.get("MccSubstepCount", 1.0)),
                    "MccSubstepFailureCount": float(r.get("MccSubstepFailureCount", 0.0)),
                    "MccAdmissibilityFailureCount": float(r.get("MccAdmissibilityFailureCount", 0.0)),
                    "MccFallbackUsed": float(r.get("MccFallbackUsed", 0.0)),
...
def plot_failed_locations(failed: list[dict[str, float | int | str]]) -> None:
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(FIGDIR / "m3d3_failed_particle_locations.png", dpi=180)
    plt.savefig(FIGDIR / "m3d3_failed_particle_locations.svg")
    plt.close()
...
def write_audit_md(
    ...
    path = ROOT / "m3d3_return_failure_audit.md"
    lines = [
...
def main() -> None:
    ...
    write_csv(ROOT / "m3d3_case_summary.csv", summaries)
    write_csv(ROOT / "m3d3_return_status_comparison.csv", all_frames)
    write_csv(ROOT / "m3d3_substep_metrics.csv", all_frames)
    write_csv(ROOT / "m3d3_admissibility_metrics.csv", all_frames)
    write_csv(ROOT / "m3d3_failed_return_particles.csv", all_failed)
    write_csv(ROOT / "m3d3_failed_particle_locations.csv", all_failed)
    write_csv(ROOT / "m3d3_return_status_by_region.csv", all_regions)
    write_csv(ROOT / "m3d3_yield_residual_comparison.csv", all_frames)
    write_csv(ROOT / "m3d3_pc_ev_plastic_strain_comparison.csv", all_frames)
    write_csv(ROOT / "m3d3_reaction_stress_path_comparison.csv", all_frames)
    write_csv(ROOT / "m3d3_pore_pressure_comparison.csv", all_frames)
    write_audit_md(all_failed, all_regions, all_frames, summaries)

    if all_frames:
        plot_series(all_frames, "status_failure_count", "m3d3_return_failure_count_vs_time", "failed return particle count")
        plot_series(all_frames, "status_line_search_count", "m3d3_line_search_failure_count_vs_time", "line-search failure count")
        plot_series(all_frames, "MccSubstepCount_mean", "m3d3_substep_count_mean_vs_time", "mean MCC substeps")
        plot_series(all_frames, "MccSubstepCount_max", "m3d3_substep_count_max_vs_time", "max MCC substeps")
        plot_series(all_frames, "MccSubstepFailureCount_sum", "m3d3_substep_failures_vs_time", "sum MCC substep failures")
        plot_series(all_frames, "MccAdmissibilityFailureCount_sum", "m3d3_admissibility_failures_vs_time", "sum admissibility failures")
        plot_series(all_frames, "yield_residual_maxAbs", "m3d3_yield_residual_vs_time", "max |yield residual|")
        plot_series(all_frames, "yield_residual_converged_maxAbs", "m3d3_converged_residual_vs_time", "max converged |yield residual|")
        plot_series(all_frames, "MccPc_mean", "m3d3_pc_mean_vs_time", "pc mean [Pa]")
        plot_series(all_frames, "MccVoidRatio_mean", "m3d3_void_ratio_vs_time", "void ratio mean")
        plot_series(all_frames, "MccPlasticVolStrain_mean", "m3d3_plastic_vol_strain_vs_time", "plastic volumetric strain mean")
        plot_series(all_frames, "MccEqPlasticStrain_max", "m3d3_eq_plastic_strain_vs_time", "max eq plastic strain")
        plot_series(all_frames, "pairwise_reaction_avg", "m3d3_pairwise_reaction_vs_axial_strain", "pairwise reaction [N]", x="axial_strain_proxy")
        plot_series(all_frames, "PorePress_mean", "m3d3_pore_pressure_vs_axial_strain", "mean pore pressure [Pa]", x="axial_strain_proxy")
        plot_series(all_frames, "Kplastic_max", "m3d3_kplastic_vs_time", "Kplastic max")
        plot_series(all_frames, "yield_fraction", "m3d3_yield_fraction_vs_time", "yield fraction")
        plot_series(all_frames, "PorePressRate_maxAbs", "m3d3_porepressrate_vs_time", "PorePressRate maxAbs [Pa/s]")
        plot_series(all_frames, "DivVel_maxAbs", "m3d3_divvel_vs_time", "DivVel maxAbs [1/s]")
        plot_series(all_frames, "velocity_max", "m3d3_velocity_max_vs_time", "velocity max [m/s]")
        plt.figure(figsize=(5.5, 5))
        for case_key in CASES:
        ...
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(FIGDIR / "m3d3_pq_path_comparison.png", dpi=180)
        plt.savefig(FIGDIR / "m3d3_pq_path_comparison.svg")
        plt.close()
