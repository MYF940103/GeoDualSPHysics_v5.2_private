#!/usr/bin/env python3
"""T4b comparison for Zhao-style renormalized flexible confinement."""
...
CASES = [
    ...
    },
    {
        ...
        "label": "renormalized, T4 loading",
        "kind": "run",
        "root": ROOT,
        "gradient_mode": 1,
    },
    {
        "case": "CaseT4b_RenormGentleLoading",
        "label": "renormalized, gentle loading",
        "kind": "run",
        "root": ROOT,
        "gradient_mode": 1,
    },
...
if raw_frame:
    ...
    summary.append({
        ...
        "max_porepressrate_abs": "", "pressure_reversal": 1, "final_kplastic_max": final["kplastic_max"],
    })
...
for case in CASES[1:]:
    ...
    means = [r["porepress_mean"] for r in frame_rows]
    reversal = any((means[i - 1] > 0 and means[i] < 0) or (means[i - 1] < 0 and means[i] > 0) for i in range(1, len(means)))
    final = frame_rows[-1]
    summary.append({
        ...
        "gradient_mode": case["gradient_mode"], "final_time": final["time"],
        ...
        "max_porepressrate_abs": max(r["porepressrate_maxabs"] for r in frame_rows),
        "pressure_reversal": int(reversal), "final_kplastic_max": final["kplastic_max"],
    })

write_table(ROOT / "t4b_case_summary.csv", summary)
write_table(ROOT / "t4b_confinement_gradient_comparison.csv", all_conf)
write_table(ROOT / "t4b_loading_stability_metrics.csv", all_frame)
write_table(ROOT / "t4b_pore_pressure_strain_metrics.csv", all_region)
write_table(ROOT / "t4b_measurement_region_metrics.csv", all_region)
...
savefig("t4b_lateral_acceleration_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_cap_leakage_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_com_acceleration_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_pore_pressure_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_pore_pressure_vs_axial_strain")
...
plt.legend(fontsize=7)
savefig("t4b_porepressrate_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_velocity_max_vs_time")
...
plt.legend(fontsize=7)
savefig("t4b_measurement_region_sensitivity")
...
plt.legend(fontsize=7)
savefig("t4b_pq_proxy")

print(f"Wrote T4b comparison metrics and figures under {ROOT}")
