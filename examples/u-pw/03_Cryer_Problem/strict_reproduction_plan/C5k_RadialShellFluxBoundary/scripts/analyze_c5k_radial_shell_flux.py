#!/usr/bin/env python3
"""Analyze C5k radial-shell / FV-consistent spherical drained boundary gate."""
...
CASES = [
    {
        "label": "mode6_shell_dp008",
        "case_name": "CaseCryer_PR_StrictSphere_C5k_FinerDiffusionShellFlux",
        "resolution": "finer",
        "dp": 0.008,
        "mode": 6,
    },
    {
        "label": "mode6_shell_dp010",
        "case_name": "CaseCryer_PR_StrictSphere_C5k_CoarseDiffusionShellFlux",
        "resolution": "coarse",
        "dp": 0.010,
        "mode": 6,
    },
...
SHELL_RE = re.compile(
    r"CPU curved drained radial-shell flux correction: TimeStep=(?P<time>[-+0-9.eE]+), bins=(?P<bins>\d+), shell_width=(?P<shell_width>[-+0-9.eE]+), "
    r"target_shell_particles=(?P<targets>\d+), sink_volume=(?P<sink_volume>[-+0-9.eE]+), fallback=(?P<fallback>\d+), "
    r"outer_population=(?P<outer_population>\d+), outer_pressure_mean=(?P<outer_pressure_mean>[-+0-9.eE]+) Pa, "
    r"outer_radius_mean=(?P<outer_radius_mean>[-+0-9.eE]+) m, boundary_value=(?P<boundary_value>[-+0-9.eE]+) Pa, "
    r"du_dr_R=(?P<du_dr_R>[-+0-9.eE]+) Pa/m, boundary_flux_density=(?P<flux_density>[-+0-9.eE]+) Pa\*m/s, "
    r"boundary_flux_integral=(?P<flux>[-+0-9.eE]+) Pa\*m3/s, storage_rate_correction=(?P<storage_rate>[-+0-9.eE]+) Pa\*m3/s, "
    r"max_abs_lap_correction=(?P<lap_corr_max>[-+0-9.eE]+), flux_storage_residual=(?P<flux_storage_residual>[-+0-9.eE]+)"
)

BINS_RE = re.compile(
    r"CPU curved drained radial-shell bins: populations=\[(?P<pop0>\d+),(?P<pop1>\d+),(?P<pop2>\d+),(?P<pop3>\d+)\], "
    r"pressure_means=\[(?P<pmean0>[-+0-9.eE]+),(?P<pmean1>[-+0-9.eE]+),(?P<pmean2>[-+0-9.eE]+),(?P<pmean3>[-+0-9.eE]+)\] Pa, "
    r"radius_means=\[(?P<rmean0>[-+0-9.eE]+),(?P<rmean1>[-+0-9.eE]+),(?P<rmean2>[-+0-9.eE]+),(?P<rmean3>[-+0-9.eE]+)\] m, "
    r"volumes=\[(?P<vol0>[-+0-9.eE]+),(?P<vol1>[-+0-9.eE]+),(?P<vol2>[-+0-9.eE]+),(?P<vol3>[-+0-9.eE]+)\] m3"
)
...
def read_runout(case: dict[str, object]) -> tuple[dict[str, object], list[dict[str, object]], dict[int, float]]:
    ...
    for i, shell in enumerate(shell_matches):
        ...
        if i < len(bin_matches):
            row.update({k: fnum(v) for k, v in bin_matches[i].groupdict().items()})
        rows.append(row)
    return meta, rows, times
...
def reference_rows() -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    return (
...
def build_comparison(frames, surfaces, shell_rows):
    ref_ts, c5i_volume, c5i_surface, c5i_summary, c5j_summary, _ = reference_rows()
...
def plot_outputs(comparison, radials, shell_rows, summary):
    figdir = ROOT / "figures"
    figdir.mkdir(exist_ok=True)
    ref_ts, c5i_volume, c5i_surface, _, _, c5j_comparison = reference_rows()
    ...
    line_compare("c5k_center_pressure_vs_fv", "center_pressure", "center_pressure", "center pressure [Pa]")
    line_compare("c5k_volume_mean_vs_fv", "volume_mean_pressure", "volume_mean_pressure", "volume mean pressure [Pa]")
    line_compare("c5k_surface_shell_vs_fv", "surface_shell_mean", "surface_shell_mean_095_100", "surface shell mean [Pa]")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5k_integrated_boundary_flux_vs_storage")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5k_radial_profiles_final")

    line_compare("c5k_porepressrate_maxabs", "porepressrate_maxAbs", "surface_flux_density", "|PorePressRate| max [Pa/s]")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5k_shell_pressure_profiles")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5k_surface_shell_p95_max_vs_time")
...
def main() -> None:
    ...
    for case in CASES:
        meta, shell_rows, times = read_runout(case)
    ...
    write_csv(ROOT / "c5k_case_summary.csv", summaries)
    write_csv(ROOT / "c5k_radial_shell_flux_diagnostics.csv", all_shell)
    write_csv(ROOT / "c5k_pressure_only_comparison.csv", comparison)
    write_csv(ROOT / "c5k_flux_ratio_metrics.csv", flux_rows)
    write_csv(ROOT / "c5k_surface_shell_metrics.csv", all_surfaces)
    write_csv(ROOT / "c5k_radial_bin_profiles.csv", all_radials)
    write_csv(ROOT / "c5k_sphere_quality_metrics.csv", all_quality)
    write_csv(ROOT / "c5k_gate_metrics.csv", gate_rows)
    plot_outputs(comparison, all_radials, all_shell, summaries)
