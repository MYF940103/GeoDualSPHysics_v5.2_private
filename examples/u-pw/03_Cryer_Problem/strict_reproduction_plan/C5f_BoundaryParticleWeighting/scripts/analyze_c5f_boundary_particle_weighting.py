#!/usr/bin/env python3
"""Analyze C5f boundary-particle weighting tests."""
...
CASES = [
    {"label": "compression_raw", "case_name": "CaseCryer_PR_StrictSphere_C5f_CompressionRaw", "kind": "compression", "mode": 4, "weighting": 0, "p0": 50.0},
    {"label": "compression_normalized", "case_name": "CaseCryer_PR_StrictSphere_C5f_CompressionNormalized", "kind": "compression", "mode": 4, "weighting": 1, "p0": 50.0},
    {"label": "compression_capped", "case_name": "CaseCryer_PR_StrictSphere_C5f_CompressionCapped", "kind": "compression", "mode": 4, "weighting": 3, "p0": 50.0},
    {"label": "diffusion_raw", "case_name": "CaseCryer_PR_StrictSphere_C5f_DiffusionRaw", "kind": "diffusion", "mode": 4, "weighting": 0, "p0": 0.0},
    {"label": "diffusion_normalized", "case_name": "CaseCryer_PR_StrictSphere_C5f_DiffusionNormalized", "kind": "diffusion", "mode": 4, "weighting": 1, "p0": 0.0},
    {"label": "diffusion_capped", "case_name": "CaseCryer_PR_StrictSphere_C5f_DiffusionCapped", "kind": "diffusion", "mode": 4, "weighting": 3, "p0": 0.0},
]
...
def generate_plots(frames: list[dict[str, object]], surfaces: list[dict[str, object]], radials: list[dict[str, object]], weights: list[dict[str, object]]) -> None:
    try:
    ...
    lines("c5f_center_normalized_pressure", frames, pk, "center excess / p0 [-]", lambda r: r["kind"] == "compression")
    lines("c5f_velocity_max", frames, "vel_max", "max velocity [m/s]")
    lines("c5f_kplastic_max", frames, "kplastic_maxAbs", "|Kplastic| max")
    lines("c5f_boundary_contribution_magnitude", frames, "surface_lap_rate_contrib_maxAbs_est", "surface LapPorePress rate contribution [Pa/s]")
    lines("c5f_pressure_only_mean_excess", frames, "excess_mean", "mean excess [Pa]", lambda r: r["kind"] == "diffusion")
    lines("c5f_surface_excess_p95_abs", surfaces, "surface_excess_p95_abs", "surface |excess| p95 [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5f_surface_excess_max_abs", surfaces, "surface_excess_maxAbs", "surface |excess| max [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5f_porepressrate_artifact_indicator", frames, "porepressrate_maxAbs", "|PorePressRate| max [Pa/s]")

    plt.figure(figsize=(7.2, 4.6))
    for label in ("compression_raw", "compression_normalized", "compression_capped", "diffusion_raw", "diffusion_normalized", "diffusion_capped"):
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(figdir / "c5f_radial_pressure_profiles.png", dpi=180)
    plt.savefig(figdir / "c5f_radial_pressure_profiles.svg")
    plt.close()
    ...
    if weight_rows:
        ...
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(figdir / "c5f_normalized_boundary_scale_summary.png", dpi=180)
        plt.savefig(figdir / "c5f_normalized_boundary_scale_summary.svg")
        plt.close()


def write_surface_audit(surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    old_final = [r for r in surfaces if r["case"] == "compression_raw" and abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9]
    if old_final:
    ...
    (ROOT / "c5f_surface_residual_audit.md").write_text("\n".join(text) + "\n")
...
def main() -> None:
    ...
    write_csv(ROOT / "c5f_case_summary.csv", summary_rows)
    write_csv(ROOT / "c5f_center_pressure_comparison.csv", frame_rows)
    write_csv(ROOT / "c5f_surface_residual_comparison.csv", surface_rows)
    write_csv(ROOT / "c5f_radial_excess_profiles.csv", radial_rows)
    write_csv(ROOT / "c5f_boundary_operator_contribution_metrics.csv", frame_rows)
    write_csv(ROOT / "c5f_boundary_particle_selection_metrics.csv", bndpart_rows)
    write_csv(ROOT / "c5f_weighting_diagnostics.csv", weight_rows)
    write_csv(ROOT / "c5f_pressure_only_diffusion_metrics.csv", [r for r in frame_rows if r["kind"] == "diffusion"])
    write_csv(ROOT / "c5f_pressure_rate_artifact_metrics.csv", frame_rows)
    write_csv(ROOT / "c5f_flexible_confining_stress_metrics.csv", conf_rows)
    write_csv(ROOT / "c5f_adami_diagnostic_metrics.csv", bndpart_rows)
    write_surface_audit(surface_rows, radial_rows)
    generate_plots(frame_rows, surface_rows, radial_rows, weight_rows)
