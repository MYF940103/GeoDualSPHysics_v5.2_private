#!/usr/bin/env python3
"""Analyze C5m conservative multi-shell spherical drained boundary gate."""
...
C5J_ROOT = PLAN_ROOT / "C5j_MLSBoundaryFlux"
C5K_ROOT = PLAN_ROOT / "C5k_RadialShellFluxBoundary"
RADIUS = 0.05
...
CASES = [
    {
        "label": "mode7_shell_exchange_dp008",
        "case_name": "CaseCryer_PR_StrictSphere_C5m_FinerDiffusionShellExchange",
        "resolution": "finer",
        "dp": 0.008,
        "mode": 7,
    },
    {
        "label": "mode7_shell_exchange_dp010",
        "case_name": "CaseCryer_PR_StrictSphere_C5m_CoarseDiffusionShellExchange",
        "resolution": "coarse",
        "dp": 0.010,
        "mode": 7,
    },
...
SHELL_RE = re.compile(
    r"CPU curved drained conservative shell exchange: TimeStep=(?P<time>[-+0-9.eE]+), shells=(?P<shells>\d+), shell_width=(?P<shell_width>[-+0-9.eE]+), "
    r"shell_mode=(?P<shell_mode>\d+), correction_mode=(?P<correction_mode>\d+), min_particles=(?P<min_particles>\d+), "
    r"corrected_particles=(?P<corrected_particles>\d+), fallback_shells=(?P<fallback_shells>\d+), min_pop=(?P<min_pop>\d+), max_pop=(?P<max_pop>\d+), "
    r"total_storage=(?P<total_storage>[-+0-9.eE]+) Pa\*m3, boundary_flux=(?P<flux>[-+0-9.eE]+) Pa\*m3/s, "
    r"target_storage_rate=(?P<storage_rate>[-+0-9.eE]+) Pa\*m3/s, current_storage_rate=(?P<current_storage_rate>[-+0-9.eE]+) Pa\*m3/s, "
    r"correction_storage_rate=(?P<correction_storage_rate>[-+0-9.eE]+) Pa\*m3/s, corrected_storage_rate=(?P<corrected_storage_rate>[-+0-9.eE]+) Pa\*m3/s, "
    r"conservation_residual=(?P<flux_storage_residual>[-+0-9.eE]+) Pa\*m3/s, max_abs_rate_correction=(?P<max_abs_rate_correction>[-+0-9.eE]+) Pa/s, "
    r"max_abs_lap_correction=(?P<lap_corr_max>[-+0-9.eE]+), min_pressure=(?P<min_pressure>[-+0-9.eE]+) Pa, max_pressure=(?P<max_pressure>[-+0-9.eE]+) Pa, "
    r"negative_count=(?P<negative_count>\d+)"
)

BINS_RE = re.compile(
    r"CPU curved drained conservative shell bin: TimeStep=(?P<time>[-+0-9.eE]+), k=(?P<k>\d+), "
    r"r_inner=(?P<r_inner>[-+0-9.eE]+), r_outer=(?P<r_outer>[-+0-9.eE]+), population=(?P<population>\d+), "
    r"effective_volume=(?P<effective_volume>[-+0-9.eE]+) m3, analytic_volume=(?P<analytic_volume>[-+0-9.eE]+) m3, "
    r"pressure_mean=(?P<pressure_mean>[-+0-9.eE]+) Pa, radius_mean=(?P<radius_mean>[-+0-9.eE]+) m, "
    r"flux_in=(?P<flux_in>[-+0-9.eE]+) Pa\*m3/s, flux_out=(?P<flux_out>[-+0-9.eE]+) Pa\*m3/s, "
    r"fv_rate=(?P<fv_rate>[-+0-9.eE]+) Pa/s, current_rate=(?P<current_rate>[-+0-9.eE]+) Pa/s, "
    r"correction_rate=(?P<correction_rate>[-+0-9.eE]+) Pa/s, lap_correction=(?P<lap_correction>[-+0-9.eE]+)"
)
...
def read_runout(case: dict[str, object]) -> tuple[dict[str, object], list[dict[str, object]], list[dict[str, object]], dict[int, float]]:
    ...
    shell_matches = list(SHELL_RE.finditer(text))
    bin_matches = list(BINS_RE.finditer(text))
    bin_rows: list[dict[str, object]] = []
    bins_by_time: dict[float, list[dict[str, object]]] = defaultdict(list)
    for match in bin_matches:
        ...
        brow.update({k: fnum(v) for k, v in match.groupdict().items()})
        brow["k"] = int(fnum(match.group("k")))
        bins_by_time[fnum(match.group("time"))].append(brow)
        bin_rows.append(brow)
    for i, shell in enumerate(shell_matches):
        row = {
            "case": case["label"],
            "case_name": case["case_name"],
            "resolution": case["resolution"],
        ...
        row.update({k: fnum(v) for k, v in shell.groupdict().items()})
        row["fallback"] = row.get("fallback_shells", 0)
        row["boundary_flux_integral"] = row.get("flux", math.nan)
        row["conservation_residual"] = row.get("flux_storage_residual", math.nan)
        bins = sorted(bins_by_time.get(fnum(row["time"]), []), key=lambda r: fnum(r["k"]))
        if bins:
            outer = bins[-1]
            inner = bins[0]
            near_outer = bins[-2] if len(bins) > 1 else bins[-1]
            row.update(
                {
                    "outer_population": outer["population"],
                    "outer_pressure_mean": outer["pressure_mean"],
                    "outer_radius_mean": outer["radius_mean"],
                    "pmean1": near_outer["pressure_mean"],
                    "pop1": near_outer["population"],
                    "inner_pressure_mean": inner["pressure_mean"],
                    "inner_population": inner["population"],
                }
            )
        rows.append(row)
    return meta, rows, bin_rows, times
...
def reference_rows() -> tuple[list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]], list[dict[str, str]]]:
    return (
        ...
        read_csv_rows(C5J_ROOT / "c5j_gate_metrics.csv"),
        read_csv_rows(C5J_ROOT / "c5j_pressure_only_comparison.csv"),
        read_csv_rows(C5K_ROOT / "c5k_gate_metrics.csv"),
        read_csv_rows(C5K_ROOT / "c5k_pressure_only_comparison.csv"),
    )


def build_comparison(frames, surfaces, shell_rows):
    ref_ts, c5i_volume, c5i_surface, c5i_summary, c5j_summary, _, c5k_summary, _ = reference_rows()
    comparison: list[dict[str, object]] = []
    flux_rows: list[dict[str, object]] = []
    surface_by_case = group_by(surfaces, "case")
    shell_by_case = group_by(shell_rows, "case")
    ...
    for case, group in group_by(frames, "case").items():
        ...
        mode5 = next((r for r in c5j_summary if abs(fnum(r["configured_dp"]) - fnum(group[0]["configured_dp"])) < 1e-12), None)
        mode6 = next((r for r in c5k_summary if abs(fnum(r["configured_dp"]) - fnum(group[0]["configured_dp"])) < 1e-12), None)
        for mrow in shell_by_case.get(case, []):
            ...
            mrow["shell_flux_ratio_to_fv"] = fnum(mrow["flux"]) / rf if abs(rf) > 1e-20 else math.nan
        yield {
            "case": case,
            ...
            "mode5_volume_rmse_same_dp": fnum(mode5["volume_mean_pressure_rmse"]) if mode5 else math.nan,
            "mode5_surface_rmse_same_dp": fnum(mode5["surface_shell_pressure_rmse"]) if mode5 else math.nan,
            "mode5_center_rmse_same_dp": fnum(mode5["center_pressure_rmse"]) if mode5 else math.nan,
            "mode5_median_flux_ratio_same_dp": fnum(mode5["median_flux_ratio"]) if mode5 else math.nan,
            "mode5_final_porepressrate_maxAbs_same_dp": fnum(mode5["final_porepressrate_maxAbs"]) if mode5 else math.nan,
            "mode6_volume_rmse_same_dp": fnum(mode6["volume_mean_pressure_rmse"]) if mode6 else math.nan,
            "mode6_surface_rmse_same_dp": fnum(mode6["surface_shell_pressure_rmse"]) if mode6 else math.nan,
            "mode6_center_rmse_same_dp": fnum(mode6["center_pressure_rmse"]) if mode6 else math.nan,
            "mode6_median_flux_ratio_same_dp": fnum(mode6["median_flux_ratio"]) if mode6 else math.nan,
            "mode6_final_porepressrate_maxAbs_same_dp": fnum(mode6["final_porepressrate_maxAbs"]) if mode6 else math.nan,
        }, comparison, flux_rows


def plot_outputs(comparison, radials, shell_rows, shell_bins, summary):
    figdir = ROOT / "figures"
    figdir.mkdir(exist_ok=True)
    ref_ts, c5i_volume, c5i_surface, _, _, c5j_comparison, _, c5k_comparison = reference_rows()
    ...
    def line_compare(name, ykey, refkey, ylabel):
        ...
        for case, group in group_by(c5j_comparison, "case").items():
            if case in ("mode5_mls_dp008", "mode5_mls_dp010"):
                group = sorted(group, key=lambda r: fnum(r["time"]))
                plt.plot([fnum(r["time"]) for r in group], [fnum(r[ykey]) for r in group], linestyle="--", label=case)
        for case, group in group_by(c5k_comparison, "case").items():
            if case in ("mode6_shell_dp008", "mode6_shell_dp010"):
                group = sorted(group, key=lambda r: fnum(r["time"]))
                plt.plot([fnum(r["time"]) for r in group], [fnum(r[ykey]) for r in group], linestyle=":", label=case)
    ...
    line_compare("c5m_center_pressure_vs_fv", "center_pressure", "center_pressure", "center pressure [Pa]")
    line_compare("c5m_volume_mean_vs_fv", "volume_mean_pressure", "volume_mean_pressure", "volume mean pressure [Pa]")
    line_compare("c5m_surface_shell_vs_fv", "surface_shell_mean", "surface_shell_mean_095_100", "surface shell mean [Pa]")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_flux_ratio_vs_time")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["flux"]) for r in group], marker="o", label=f"{case} boundary flux")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_integrated_boundary_flux_vs_storage")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_radial_profiles_final")

    line_compare("c5m_porepressrate_maxabs", "porepressrate_maxAbs", "surface_flux_density", "|PorePressRate| max [Pa/s]")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["outer_pressure_mean"]) for r in group], marker="o", label=f"{case} outer mean")
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["pmean1"]) for r in group], marker="s", linestyle="--", label=f"{case} shell 2 mean")
    plt.xlabel("time [s]")
    plt.ylabel("shell pressure [Pa]")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_shell_pressure_profiles")
    ...
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_surface_shell_p95_max_vs_time")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["outer_population"]) for r in group], marker="o", label=f"{case} outer")
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["pop1"]) for r in group], marker="s", linestyle="--", label=f"{case} shell 2")
    plt.xlabel("time [s]")
    plt.ylabel("shell population")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_shell_populations")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_bins, "case").items():
        last_k = max(int(fnum(r["k"])) for r in group)
        for k in sorted({int(fnum(r["k"])) for r in group if int(fnum(r["k"])) in (max(0, last_k - 1), last_k)}):
            kg = sorted([r for r in group if int(fnum(r["k"])) == k], key=lambda r: fnum(r["time"]))
            plt.plot([fnum(r["time"]) for r in kg], [fnum(r["flux_out"]) for r in kg], marker="o", label=f"{case} F_out shell {k}")
    plt.xlabel("time [s]")
    plt.ylabel("interface flux [Pa*m3/s]")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_interface_flux_vs_time")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [abs(fnum(r["flux_storage_residual"])) for r in group], marker="o", label=case)
    plt.xlabel("time [s]")
    plt.ylabel("|storage + boundary flux| [Pa*m3/s]")
    plt.yscale("symlog", linthresh=1e-12)
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_shell_storage_balance_residual")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        times = np.array([fnum(r["time"]) for r in group], dtype=float)
        flux = np.array([fnum(r["flux"]) for r in group], dtype=float)
        total = np.array([fnum(r["total_storage"]) for r in group], dtype=float)
        cum = np.zeros_like(times)
        if len(times) > 1:
            cum[1:] = np.cumsum(0.5 * (flux[1:] + flux[:-1]) * np.diff(times))
        plt.plot(times, total[0] - total, marker="o", label=f"{case} storage loss")
        plt.plot(times, cum, linestyle="--", label=f"{case} integrated flux")
    plt.xlabel("time [s]")
    plt.ylabel("pressure storage change [Pa*m3]")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_total_storage_vs_integrated_flux")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["flux_storage_residual"]) for r in group], marker="o", label=case)
    plt.xlabel("time [s]")
    plt.ylabel("conservation residual [Pa*m3/s]")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_conservation_residual_vs_time")

    plt.figure(figsize=(7.4, 4.6))
    for case, group in group_by(shell_rows, "case").items():
        group = sorted(group, key=lambda r: fnum(r["time"]))
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["negative_count"]) for r in group], marker="o", label=f"{case} negative count")
        plt.plot([fnum(r["time"]) for r in group], [fnum(r["min_pressure"]) for r in group], linestyle="--", label=f"{case} min pressure")
    plt.xlabel("time [s]")
    plt.ylabel("negative count / min pressure [Pa]")
    plt.grid(True, alpha=0.25)
    plt.legend(fontsize=8)
    save("c5m_negative_pressure_indicator")
...
def main() -> None:
    ...
    all_shell: list[dict[str, object]] = []
    all_shell_bins: list[dict[str, object]] = []
    summaries: list[dict[str, object]] = []
    for case in CASES:
        meta, shell_rows, shell_bin_rows, times = read_runout(case)
        ...
        all_shell.extend(shell_rows)
        all_shell_bins.extend(shell_bin_rows)
        summary = dict(meta)
        summary.update(case)
    ...
    comparison: list[dict[str, object]] = []
    flux_rows: list[dict[str, object]] = []
    for summary, comp, flux in build_comparison(all_frames, all_surfaces, all_shell):
    ...
    for row in summaries:
        ...
        row["gate_volume_improved_vs_mode5"] = fnum(gate.get("volume_mean_pressure_rmse")) < fnum(gate.get("mode5_volume_rmse_same_dp"))
        row["gate_surface_improved_vs_mode5"] = fnum(gate.get("surface_shell_pressure_rmse")) < fnum(gate.get("mode5_surface_rmse_same_dp"))
        row["gate_center_improved_vs_mode5"] = fnum(gate.get("center_pressure_rmse")) < fnum(gate.get("mode5_center_rmse_same_dp"))
        row["gate_flux_closer_to_one_vs_mode5"] = abs(fnum(gate.get("median_flux_ratio")) - 1.0) < abs(fnum(gate.get("mode5_median_flux_ratio_same_dp")) - 1.0)
        row["gate_porepressrate_not_worse_than_mode5"] = fnum(gate.get("final_porepressrate_maxAbs")) <= 1.05 * fnum(gate.get("mode5_final_porepressrate_maxAbs_same_dp"))
        row["gate_volume_improved_vs_mode6"] = fnum(gate.get("volume_mean_pressure_rmse")) < fnum(gate.get("mode6_volume_rmse_same_dp"))
        row["gate_surface_improved_vs_mode6"] = fnum(gate.get("surface_shell_pressure_rmse")) < fnum(gate.get("mode6_surface_rmse_same_dp"))
        row["gate_center_improved_vs_mode6"] = fnum(gate.get("center_pressure_rmse")) < fnum(gate.get("mode6_center_rmse_same_dp"))
        row["gate_flux_closer_to_one_vs_mode6"] = abs(fnum(gate.get("median_flux_ratio")) - 1.0) < abs(fnum(gate.get("mode6_median_flux_ratio_same_dp")) - 1.0)
        row["gate_porepressrate_not_worse_than_mode6"] = fnum(gate.get("final_porepressrate_maxAbs")) <= 1.05 * fnum(gate.get("mode6_final_porepressrate_maxAbs_same_dp"))
        row["pressure_only_gate_pass"] = (
            ...
            and row["gate_volume_improved_vs_mode5"]
            and row["gate_surface_improved_vs_mode5"]
            and row["gate_center_improved_vs_mode5"]
            and row["gate_flux_closer_to_one_vs_mode5"]
            and row["gate_porepressrate_not_worse_than_mode5"]
            and row["gate_volume_improved_vs_mode6"]
            and row["gate_surface_improved_vs_mode6"]
            and row["gate_center_improved_vs_mode6"]
            and row["gate_flux_closer_to_one_vs_mode6"]
            and row["gate_porepressrate_not_worse_than_mode6"]
            and not bool(row.get("flux_reversal"))
            and not bool(row.get("negative_pressure"))
        )

    write_csv(ROOT / "c5m_case_summary.csv", summaries)
    write_csv(ROOT / "c5m_shell_exchange_diagnostics.csv", all_shell)
    write_csv(ROOT / "c5m_shell_bin_diagnostics.csv", all_shell_bins)
    write_csv(ROOT / "c5m_pressure_only_comparison.csv", comparison)
    write_csv(ROOT / "c5m_flux_ratio_metrics.csv", flux_rows)
    write_csv(ROOT / "c5m_surface_shell_metrics.csv", all_surfaces)
    write_csv(ROOT / "c5m_radial_bin_profiles.csv", all_radials)
    write_csv(ROOT / "c5m_sphere_quality_metrics.csv", all_quality)
    write_csv(ROOT / "c5m_gate_metrics.csv", gate_rows)
    plot_outputs(comparison, all_radials, all_shell, all_shell_bins, summaries)
