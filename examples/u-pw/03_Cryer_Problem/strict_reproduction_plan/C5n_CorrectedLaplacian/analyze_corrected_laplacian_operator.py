#!/usr/bin/env python3
"""C5n corrected Laplacian audit and pressure-only gate for strict Cryer."""
...
CASE = {
    "label": "mode8_corrected_laplacian_dp008",
    "case_name": "CaseCryer_PR_StrictSphere_C5n_FinerDiffusionCorrectedLap",
    "resolution": "finer",
    "dp": 0.008,
    "mode": 8,
}
...
def audit_manufactured() -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    operators = ["material_only", "mode8_exact_boundary", "mode8_drained_zero_boundary"]
    ...
    write_csv(ROOT / "c5n_manufactured_laplacian_summary.csv", summary)
    write_csv(ROOT / "c5n_manufactured_laplacian_by_shell.csv", shell_rows)
    write_csv(ROOT / "c5n_manufactured_laplacian_particles_sample.csv", sample_rows)
    write_csv(ROOT / "c5n_corrected_laplacian_static_diagnostics.csv", diag_rows)
...
def plot_manufactured(shell_rows: list[dict[str, object]], diag_rows: list[dict[str, object]]) -> None:
    for field, title, name in [
        ("constant", "constant field residual", "c5n_constant_laplacian_residual"),
        ("quadratic_r2", "quadratic field error", "c5n_quadratic_laplacian_error"),
        ("linear_gap", "drained-like gap error", "c5n_linear_gap_laplacian_error"),
    ]:
        plt.figure(figsize=(7, 4))
        for op in ["material_only", "mode8_exact_boundary", "mode8_drained_zero_boundary"]:
    ...
    savefig("c5n_condition_number_stats")
...
def parse_runout() -> tuple[dict[str, object], list[dict[str, object]], dict[int, float]]:
    ...
    for line in text.splitlines():
        ...
        for key, value in m.groupdict().items():
            row[key] = fnum(value)
        rows.append(row)
    write_csv(ROOT / "c5n_corrected_laplacian_runtime_diagnostics.csv", rows)
    return meta, rows, times
...
def write_boundary_type_note() -> None:
    text = """# C5n Boundary Type Diagnosis
...
"""
    (ROOT / "c5n_boundary_type_diagnosis.md").write_text(text, encoding="utf-8")


def main() -> None:
    summary, shell_rows, _sample_rows, static_diag = audit_manufactured()
    plot_manufactured(shell_rows, static_diag)
    meta, runtime_diag, times = parse_runout()
    analyze_pressure(meta, runtime_diag, times)
    write_boundary_type_note()
    print("C5n analysis complete.")
