#!/usr/bin/env python3
"""Analyze C5e boundary-particle drained boundary tests."""
...
CASES = [
    {"label": "mode3_shell", "case_name": "CaseCryer_PR_StrictSphere_C5e_Mode3Shell", "kind": "compression", "mode": 3, "p0": 50.0},
    {"label": "mode4_boundary_particles", "case_name": "CaseCryer_PR_StrictSphere_C5e_Mode4BoundaryParticles", "kind": "compression", "mode": 4, "p0": 50.0},
    {"label": "diffusion_mode3_shell", "case_name": "CaseCryer_PR_StrictSphere_C5e_DiffusionMode3Shell", "kind": "diffusion", "mode": 3, "p0": 0.0},
    {"label": "diffusion_mode4_boundary_particles", "case_name": "CaseCryer_PR_StrictSphere_C5e_DiffusionMode4BoundaryParticles", "kind": "diffusion", "mode": 4, "p0": 0.0},
]
...
def read_runout(case_name: str) -> tuple[dict[str, object], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], dict[int, float]]:
    ...
    if not runout.exists():
        return meta, curved, conf, clamps, quads, bndparts, times
...
def generate_plots(frames: list[dict[str, object]], surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    ...
    lines("c5e_center_normalized_pressure_mode3_vs_mode4", frames, pk, "center excess / p0 [-]", lambda r: r["kind"] == "compression")
    lines("c5e_velocity_max", frames, "vel_max", "max velocity [m/s]")
    lines("c5e_kplastic_max", frames, "kplastic_maxAbs", "|Kplastic| max")
    lines("c5e_boundary_contribution_magnitude", frames, "surface_lap_rate_contrib_maxAbs_est", "surface LapPorePress rate contribution [Pa/s]")
    lines("c5e_pressure_only_diffusion_mean_excess", frames, "excess_mean", "mean excess [Pa]", lambda r: r["kind"] == "diffusion")
    lines("c5e_surface_excess_p95_abs", surfaces, "surface_excess_p95_abs", "surface |excess| p95 [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5e_surface_excess_max_abs", surfaces, "surface_excess_maxAbs", "surface |excess| max [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5e_porepressrate_artifact_indicator", frames, "porepressrate_maxAbs", "|PorePressRate| max [Pa/s]")

    plt.figure(figsize=(7.2, 4.6))
    for label in ("mode3_shell", "mode4_boundary_particles", "diffusion_mode3_shell", "diffusion_mode4_boundary_particles"):
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(figdir / "c5e_radial_pressure_profiles_mode3_vs_mode4.png", dpi=180)
    plt.savefig(figdir / "c5e_radial_pressure_profiles_mode3_vs_mode4.svg")
    plt.close()


def write_surface_audit(surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    old_final = [r for r in surfaces if r["case"] == "mode3_shell" and abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9]
    if old_final:
    ...
    text = [
        ...
        "",
        f"For the mode-3 shell control at the final retained frame (`t={tmax:g} s`), the `r>0.85R` surface layer has mean excess `{mean:g} Pa`, p95 absolute excess `{p95:g} Pa`, and max absolute excess `{mx:g} Pa`.",
        "",
    ...
    (ROOT / "c5e_surface_residual_audit.md").write_text("\n".join(text) + "\n")
...
def main() -> None:
    ...
    for case in CASES:
        meta, curved, conf, clamps, quads, bndparts, times = read_runout(str(case["case_name"]))
    ...
    write_csv(ROOT / "c5e_case_summary.csv", summary_rows)
    write_csv(ROOT / "c5e_center_pressure_comparison.csv", frame_rows)
    write_csv(ROOT / "c5e_surface_residual_comparison.csv", surface_rows)
    write_csv(ROOT / "c5e_radial_excess_profiles.csv", radial_rows)
    write_csv(ROOT / "c5e_boundary_operator_contribution_metrics.csv", frame_rows)
    write_csv(ROOT / "c5e_boundary_particle_selection_metrics.csv", bndpart_rows)
    write_csv(ROOT / "c5e_pressure_only_diffusion_metrics.csv", [r for r in frame_rows if r["kind"] == "diffusion"])
    write_csv(ROOT / "c5e_pressure_rate_artifact_metrics.csv", frame_rows)
    write_csv(ROOT / "c5e_flexible_confining_stress_metrics.csv", conf_rows)
    write_csv(ROOT / "c5e_adami_diagnostic_metrics.csv", bndpart_rows)
