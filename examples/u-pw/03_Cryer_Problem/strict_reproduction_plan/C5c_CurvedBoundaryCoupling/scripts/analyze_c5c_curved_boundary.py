#!/usr/bin/env python3
"""Analyze C5c curved drained boundary coupling tests."""
...
CASES = [
    {"label": "old_ghost", "case_name": "CaseCryer_PR_StrictSphere_C5c_OldGhost", "kind": "compression", "mode": 0, "p0": 50.0},
    {"label": "strong_ghost", "case_name": "CaseCryer_PR_StrictSphere_C5c_StrongGhost", "kind": "compression", "mode": 1, "p0": 50.0},
    {"label": "surface_clamp", "case_name": "CaseCryer_PR_StrictSphere_C5c_SurfaceClamp", "kind": "compression", "mode": 2, "p0": 50.0},
    {"label": "diffusion_old", "case_name": "CaseCryer_PR_StrictSphere_C5c_DiffusionOld", "kind": "diffusion", "mode": 0, "p0": 0.0},
    {"label": "diffusion_strong", "case_name": "CaseCryer_PR_StrictSphere_C5c_DiffusionStrong", "kind": "diffusion", "mode": 1, "p0": 0.0},
    {"label": "diffusion_clamp", "case_name": "CaseCryer_PR_StrictSphere_C5c_DiffusionClamp", "kind": "diffusion", "mode": 2, "p0": 0.0},
]
...
def read_runout(case_name: str) -> tuple[dict[str, object], list[dict[str, object]], list[dict[str, object]], list[dict[str, object]], dict[int, float]]:
    ...
    if not runout.exists():
        return meta, curved, conf, clamps, times
...
def generate_plots(frames: list[dict[str, object]], surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    ...
    lines("c5c_center_normalized_pressure_old_vs_new", frames, pk, "center excess / p0 [-]", lambda r: r["kind"] == "compression")
    lines("c5c_velocity_max", frames, "vel_max", "max velocity [m/s]")
    lines("c5c_kplastic_max", frames, "kplastic_maxAbs", "|Kplastic| max")
    lines("c5c_boundary_contribution_magnitude", frames, "surface_lap_rate_contrib_maxAbs_est", "surface LapPorePress rate contribution [Pa/s]")
    lines("c5c_pressure_only_diffusion_mean_excess", frames, "excess_mean", "mean excess [Pa]", lambda r: r["kind"] == "diffusion")
    lines("c5c_diagnostic_clamp_center_effect", frames, pk, "center excess / p0 [-]", lambda r: r["case"] in ("old_ghost", "strong_ghost", "surface_clamp"))
    lines("c5c_surface_excess_p95_abs", surfaces, "surface_excess_p95_abs", "surface |excess| p95 [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5c_surface_excess_max_abs", surfaces, "surface_excess_maxAbs", "surface |excess| max [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)

    plt.figure(figsize=(7.2, 4.6))
    for label in ("old_ghost", "strong_ghost", "surface_clamp"):
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(figdir / "c5c_radial_pressure_profiles_old_vs_new.png", dpi=180)
    plt.savefig(figdir / "c5c_radial_pressure_profiles_old_vs_new.svg")
    plt.close()
...
def write_surface_audit(surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    ...
    text = [
        ...
        "",
        "The detailed particle-field audit was rerun under the C5c old-ghost case because C5b retained only frame summaries after raw-output cleanup.",
        "",
    ...
    (ROOT / "c5c_surface_residual_audit.md").write_text("\n".join(text) + "\n")
...
def main() -> None:
    ...
    for case in CASES:
        meta, curved, conf, clamps, times = read_runout(str(case["case_name"]))
    ...
    write_csv(ROOT / "c5c_case_summary.csv", summary_rows)
    write_csv(ROOT / "c5c_center_pressure_comparison.csv", frame_rows)
    write_csv(ROOT / "c5c_surface_residual_comparison.csv", surface_rows)
    write_csv(ROOT / "c5c_surface_residual_audit.csv", surface_rows)
    write_csv(ROOT / "c5c_radial_excess_bins.csv", radial_rows)
    write_csv(ROOT / "c5c_radial_excess_profiles.csv", radial_rows)
    write_csv(ROOT / "c5c_boundary_contribution_metrics.csv", frame_rows)
    write_csv(ROOT / "c5c_pressure_only_diffusion_metrics.csv", [r for r in frame_rows if r["kind"] == "diffusion"])
    write_csv(ROOT / "c5c_flexible_confining_stress_metrics.csv", conf_rows)
    write_csv(ROOT / "c5c_clamp_diagnostics.csv", clamp_rows)
