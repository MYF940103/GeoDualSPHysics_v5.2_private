#!/usr/bin/env python3
"""Analyze C4-D no-elevation hydraulic smoke outputs."""
...
INTERIOR_FRAC = 0.50
CASES = [
    ("source_on_regression", "CaseC4D_SourceOn_Regression"),
    ("no_elevation_diffusion", "CaseC4D_NoElevation_Diffusion"),
    ("no_elevation_compression", "CaseC4D_NoElevation_Compression"),
]
...
def plot(all_rows: list[dict[str, object]], residual_rows: list[dict[str, object]]) -> None:
    ...
    lineplot("c4d_boundary_excess_residual_proxy", "surface_ExcessPorePress_maxAbs", "surface |excess| max [Pa]")
    lineplot("c4d_center_excess_pressure", "center_ExcessPorePress", "center excess pressure [Pa]")
    lineplot("c4d_mean_excess_pressure", "ExcessPorePress_mean", "mean excess pressure [Pa]")
    lineplot("c4d_excess_pressure_maxabs", "ExcessPorePress_maxAbs", "|excess| max [Pa]")
    lineplot("c4d_porepress_rate_maxabs", "PorePressRate_maxAbs", "|PorePressRate| max [Pa/s]")
    lineplot("c4d_lapporepress_maxabs", "LapPorePress_maxAbs", "|LapPorePress| max")
    lineplot("c4d_lapz_maxabs", "LapZ_maxAbs", "|LapZ| max (diagnostic)")
    lineplot("c4d_velocity_max", "vel_max", "max velocity [m/s]")
    lineplot("c4d_kplastic_maxabs", "Kplastic_maxAbs", "|Kplastic| max")
    lineplot("c4d_surface_radial_velocity", "surface_radial_vel_mean", "surface radial velocity mean [m/s]")
    ...
    if residual_rows:
        ...
        plt.title("C4-D curved drained operator residual")
        plt.legend()
        plt.tight_layout()
        plt.savefig(fig_dir / "c4d_operator_boundary_residual.png", dpi=180)
        plt.savefig(fig_dir / "c4d_operator_boundary_residual.svg")
        plt.close()
...
def main() -> None:
    ...
    for label, case_name in CASES:
        ...
        for row in merged:
            row["case"] = label
            row["HydraulicElevationSource"] = 1 if label == "source_on_regression" else 0
            # Source-on regression uses k=0 and no-elevation cases omit k*LapZ,
            # so the used elevation-source contribution is zero in all C4-D smokes.
            row["LapZ_rate_contribution_maxAbs_est"] = 0.0
            all_rows.append(row)
        for row in curved:
            row["case"] = label
    ...
    common_fields = [
        ...
        "DivVel_maxAbs", "HydraulicElevationSource",
    ...
    write_csv(ROOT / "c4d_hydraulic_no_elevation_case_summary.csv", summary_rows, summary_fields)
    write_csv(ROOT / "c4d_hydraulic_scaling_metrics.csv", [r for r in all_rows if r["case"] in ("source_on_regression", "no_elevation_diffusion")], common_fields)
    write_csv(ROOT / "c4d_lapz_contribution_check.csv", all_rows, common_fields)
    write_csv(ROOT / "c4d_compression_drainage_metrics.csv", [r for r in all_rows if r["case"] == "no_elevation_compression"], common_fields)
    write_csv(ROOT / "c4d_confining_stress_diagnostics.csv", conf_rows, conf_fields)
    write_csv(ROOT / "c4d_drained_boundary_residual_metrics.csv", residual_rows, residual_fields)
