#!/usr/bin/env python3
"""Analyze C5 coarse strict-sphere Cryer smoke outputs."""
...
CENTER_AVG_RADIUS = 0.02
SURFACE_FRAC = 0.85
INTERIOR_FRAC = 0.50
CASES = [
    ("c5_0p006", "CaseCryer_PR_StrictSphere_C5_CoarseSmoke"),
]
...
def read_partcsv(case_name: str, times: dict[int, float]) -> list[dict[str, object]]:
    ...
    for iframe, path in enumerate(sorted(data_dir.glob("PartCsv_*.csv"))):
        ...
        center_p = math.nan
        center_r = math.inf
        center_avg_p: list[float] = []
        center_avg_ex: list[float] = []
        surf_ex: list[float] = []
        ...
        for line in lines[header_idx + 1:]:
            ...
            if idx["x"] is not None and idx["y"] is not None and idx["z"] is not None:
                ...
                if r < center_r:
                    ...
                    center_p = rowvals["p"]
                    center_ex = ex
                if r <= CENTER_AVG_RADIUS:
                    if math.isfinite(rowvals["p"]):
                        center_avg_p.append(rowvals["p"])
                    if math.isfinite(ex):
                        center_avg_ex.append(ex)
        ...
        rows.append({
            ...
            "Tv_est": CV_EST * times.get(iframe, float(iframe)) / (RADIUS * RADIUS),
            ...
            "center_avg_radius": CENTER_AVG_RADIUS,
            "center_avg_count": len(center_avg_ex),
            "center_avg_PorePress": sum(center_avg_p) / len(center_avg_p) if center_avg_p else math.nan,
            "center_avg_ExcessPorePress": sum(center_avg_ex) / len(center_avg_ex) if center_avg_ex else math.nan,
            "center_avg_ExcessPorePress_over_p0": (sum(center_avg_ex) / len(center_avg_ex)) / P0 if center_avg_ex and P0 else math.nan,
...
def plot(all_rows: list[dict[str, object]], residual_rows: list[dict[str, object]]) -> None:
    ...
    lineplot("c5_boundary_excess_residual_proxy", "surface_ExcessPorePress_maxAbs", "surface |excess| max [Pa]")
    lineplot("c5_center_excess_pressure", "center_ExcessPorePress", "nearest center excess pressure [Pa]")
    lineplot("c5_center_avg_excess_pressure", "center_avg_ExcessPorePress", "averaged center excess pressure [Pa]")
    lineplot("c5_center_normalized_excess_pressure", "center_avg_ExcessPorePress_over_p0", "averaged center excess / p0 [-]")
    lineplot("c5_mean_excess_pressure", "ExcessPorePress_mean", "mean excess pressure [Pa]")
    lineplot("c5_excess_pressure_maxabs", "ExcessPorePress_maxAbs", "|excess| max [Pa]")
    lineplot("c5_porepress_rate_maxabs", "PorePressRate_maxAbs", "|PorePressRate| max [Pa/s]")
    lineplot("c5_lapporepress_maxabs", "LapPorePress_maxAbs", "|LapPorePress| max")
    lineplot("c5_lapz_maxabs", "LapZ_maxAbs", "|LapZ| max (diagnostic only)")
    lineplot("c5_divvel_rate_contribution", "DivVel_rate_contribution_maxAbs_est", "|Kw/n DivVel| max [Pa/s]")
    lineplot("c5_lapp_rate_contribution", "LapPorePress_rate_contribution_maxAbs_est", "|Kw/n k/(rho g) LapP| max [Pa/s]")
    lineplot("c5_lapz_rate_contribution", "LapZ_rate_contribution_maxAbs_est", "used |Kw/n k LapZ| max [Pa/s]")
    lineplot("c5_velocity_max", "vel_max", "max velocity [m/s]")
    lineplot("c5_kplastic_maxabs", "Kplastic_maxAbs", "|Kplastic| max")
    lineplot("c5_surface_radial_velocity", "surface_radial_vel_mean", "surface radial velocity mean [m/s]")
    lineplot("c5_surface_radial_displacement", "surface_radial_disp_mean", "surface radial displacement mean [m]")
    ...
    if residual_rows:
        ...
        plt.title("C5 curved drained operator residual")
        plt.legend()
        plt.tight_layout()
        plt.savefig(fig_dir / "c5_operator_boundary_residual.png", dpi=180)
        plt.savefig(fig_dir / "c5_operator_boundary_residual.svg")
        plt.close()
...
def main() -> None:
    ...
    for label, case_name in CASES:
        ...
        for row in merged:
            row["case"] = label
            row["HydraulicElevationSource"] = 0
            all_rows.append(row)
        for row in curved:
            row["case"] = label
        ...
        center_rows = [r for r in merged if str(r.get("center_avg_ExcessPorePress", "")) != ""]
        peak_center = max((fnum(r.get("center_avg_ExcessPorePress")) for r in center_rows), default=math.nan)
        peak_row = max(center_rows, key=lambda r: fnum(r.get("center_avg_ExcessPorePress")), default={})
        summary_rows.append({
            "case": label,
            "code": meta.get("code", ""),
            ...
            "final_center_avg_excess": final.get("center_avg_ExcessPorePress", ""),
            "final_center_avg_excess_over_p0": final.get("center_avg_ExcessPorePress_over_p0", ""),
    ...
    common_fields = [
        ...
        "LapPorePress_maxAbs", "LapZ_maxAbs", "LapZ_rate_contribution_maxAbs_est",
        "DivVel_maxAbs", "DivVel_rate_contribution_maxAbs_est",
        "LapPorePress_rate_contribution_maxAbs_est", "LapZ_rate_contribution_maxAbs_est",
        "HydraulicElevationSource",
        "center_PorePress", "center_ExcessPorePress", "center_PorePress_over_p0",
        "center_ExcessPorePress_over_p0", "center_avg_radius", "center_avg_count",
        "center_avg_PorePress", "center_avg_ExcessPorePress",
        "center_avg_ExcessPorePress_over_p0", "center_radius",
    ...
    summary_fields = [
        ...
        "particle_count", "curved_diag_rows", "final_affected",
        "final_boundary_residual_max", "final_p0_eff",
        "peak_center_avg_excess", "peak_center_avg_excess_over_p0",
        "peak_time", "peak_Tv_est", "final_vel_max", "final_center_excess",
        "final_center_avg_excess", "final_center_avg_excess_over_p0",
        "final_mean_excess", "final_surface_excess_maxAbs",
        "final_porepressrate_maxAbs", "final_lapporepress_maxAbs",
        "final_kplastic_maxAbs",
    ]
    write_csv(ROOT / "c5_strict_sphere_case_summary.csv", summary_rows, summary_fields)
    write_csv(ROOT / "c5_center_pressure_history.csv", all_rows, common_fields)
    write_csv(ROOT / "c5_boundary_residual_metrics.csv", residual_rows, residual_fields)
    write_csv(ROOT / "c5_flexible_confining_stress_metrics.csv", conf_rows, conf_fields)
    write_csv(ROOT / "c5_pressure_rate_terms.csv", all_rows, common_fields)
