#!/usr/bin/env python3
"""Analyze C5h higher-resolution sphere diagnostics."""
...
CASES = [
    {"label": "higher_compression", "case_name": "CaseCryer_PR_StrictSphere_C5h_HigherResCompressionNormalized", "kind": "compression", "resolution": "higher", "dp": 0.0065, "mode": 4, "weighting": 1, "p0": 50.0},
    {"label": "higher_diffusion", "case_name": "CaseCryer_PR_StrictSphere_C5h_HigherResDiffusionNormalized", "kind": "diffusion", "resolution": "higher", "dp": 0.0065, "mode": 4, "weighting": 1, "p0": 0.0},
]

PREVIOUS_ROOT = ROOT.parent / "C5g_GeometryDpDiagnostic"
...
def merged_with_c5g(stem: str, rows: list[dict[str, object]]) -> list[dict[str, object]]:
    return read_csv_rows(PREVIOUS_ROOT / f"c5g_{stem}.csv") + rows


def one_geometry_row_per_resolution(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    out: list[dict[str, object]] = []
    seen: set[str] = set()
    for row in rows:
        if row.get("kind") != "compression":
            continue
        key = str(row.get("resolution", row.get("case", "")))
        if key in seen:
            continue
        seen.add(key)
        out.append(row)
...
def generate_plots(frames: list[dict[str, object]], surfaces: list[dict[str, object]], radials: list[dict[str, object]], weights: list[dict[str, object]], quality: list[dict[str, object]]) -> None:
    ...
    lines("c5h_center_normalized_pressure", frames, pk, "center excess / p0 [-]", lambda r: r["kind"] == "compression")
    lines("c5h_velocity_max", frames, "vel_max", "max velocity [m/s]")
    lines("c5h_kplastic_max", frames, "kplastic_maxAbs", "|Kplastic| max")
    lines("c5h_boundary_contribution_magnitude", frames, "surface_lap_rate_contrib_maxAbs_est", "surface LapPorePress rate contribution [Pa/s]")
    lines("c5h_pressure_only_mean_excess", frames, "excess_mean", "mean excess [Pa]", lambda r: r["kind"] == "diffusion")
    lines("c5h_surface_excess_p95_abs", surfaces, "surface_excess_p95_abs", "surface |excess| p95 [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5h_surface_excess_max_abs", surfaces, "surface_excess_maxAbs", "surface |excess| max [Pa]", lambda r: abs(fnum(r["surface_threshold"]) - 0.85) < 1e-9)
    lines("c5h_porepressrate_artifact_indicator", frames, "porepressrate_maxAbs", "|PorePressRate| max [Pa/s]")

    plt.figure(figsize=(7.2, 4.6))
    for label in sorted({str(r["case"]) for r in radials}):
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(figdir / "c5h_radial_pressure_profiles.png", dpi=180)
    plt.savefig(figdir / "c5h_radial_pressure_profiles.svg")
    plt.close()
    ...
    if weight_rows:
        ...
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(figdir / "c5h_normalized_boundary_scale_summary.png", dpi=180)
        plt.savefig(figdir / "c5h_normalized_boundary_scale_summary.svg")
        plt.close()
    ...
    if quality:
        ...
        plt.grid(True, axis="y", alpha=0.25)
        plt.tight_layout()
        plt.savefig(figdir / "c5h_sphere_particle_count.png", dpi=180)
        plt.savefig(figdir / "c5h_sphere_particle_count.svg")
        plt.close()
        ...
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(figdir / "c5h_sphere_quality_summary.png", dpi=180)
        plt.savefig(figdir / "c5h_sphere_quality_summary.svg")
        plt.close()

    plt.figure(figsize=(7.4, 4.6))
    for label in sorted({str(r["case"]) for r in frames if r.get("kind") == "compression"}):
    ...
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(figdir / "c5h_center_averaging_sensitivity.png", dpi=180)
    plt.savefig(figdir / "c5h_center_averaging_sensitivity.svg")
    plt.close()
...
def write_surface_audit(surfaces: list[dict[str, object]], radials: list[dict[str, object]]) -> None:
    ...
    if old_final:
        ...
        verdict = "unknown"
    text = [
        "# C5h Surface Residual Audit",
        "",
        "The detailed particle-field audit compares the C5g coarse/finer and C5h higher-resolution normalized mode-4 sphere diagnostics.",
        "",
        f"For the coarse control at the final retained frame (`t={tmax:g} s`), the `r>0.85R` surface layer has mean excess `{mean:g} Pa`, p95 absolute excess `{p95:g} Pa`, and max absolute excess `{mx:g} Pa`.",
        "",
    ...
    (ROOT / "c5h_surface_residual_audit.md").write_text("\n".join(text) + "\n")
...
def main() -> None:
    ...
    all_summary_rows = merged_with_c5g("case_summary", summary_rows)
    all_quality_rows = merged_with_c5g("sphere_quality_metrics", quality_rows)
    all_frame_rows = merged_with_c5g("center_pressure_comparison", frame_rows)
    all_surface_rows = merged_with_c5g("surface_residual_comparison", surface_rows)
    all_center_sensitivity_rows = merged_with_c5g("center_averaging_sensitivity", center_sensitivity_rows)
    all_radial_rows = merged_with_c5g("radial_profile_metrics", radial_rows)
    all_bndpart_rows = merged_with_c5g("boundary_selection_metrics", bndpart_rows)
    all_weight_rows = merged_with_c5g("weighting_diagnostics", weight_rows)
    all_conf_rows = merged_with_c5g("flexible_confining_stress_metrics", conf_rows)

    write_csv(ROOT / "c5h_case_summary.csv", all_summary_rows)
    write_csv(ROOT / "c5h_sphere_quality_metrics.csv", all_quality_rows)
    write_csv(ROOT / "c5h_geometry_resolution_comparison.csv", one_geometry_row_per_resolution(all_quality_rows))
    write_csv(ROOT / "c5h_geometry_comparison_metrics.csv", all_quality_rows)
    write_csv(ROOT / "c5h_center_pressure_comparison.csv", all_frame_rows)
    write_csv(ROOT / "c5h_surface_residual_comparison.csv", all_surface_rows)
    write_csv(ROOT / "c5h_center_averaging_sensitivity.csv", all_center_sensitivity_rows)
    write_csv(ROOT / "c5h_radial_profile_metrics.csv", all_radial_rows)
    write_csv(ROOT / "c5h_boundary_selection_metrics.csv", all_bndpart_rows)
    write_csv(ROOT / "c5h_boundary_operator_contribution_metrics.csv", all_frame_rows)
    write_csv(ROOT / "c5h_weighting_diagnostics.csv", all_weight_rows)
    write_csv(ROOT / "c5h_pressure_only_diffusion_comparison.csv", [r for r in all_frame_rows if r["kind"] == "diffusion"])
    write_csv(ROOT / "c5h_pressure_only_diffusion_metrics.csv", [r for r in all_frame_rows if r["kind"] == "diffusion"])
    write_csv(ROOT / "c5h_pressure_rate_artifact_metrics.csv", all_frame_rows)
    write_csv(ROOT / "c5h_flexible_confining_stress_metrics.csv", all_conf_rows)
    write_csv(ROOT / "c5h_adami_diagnostic_metrics.csv", all_bndpart_rows)
    write_surface_audit(all_surface_rows, all_radial_rows)
    generate_plots(all_frame_rows, all_surface_rows, all_radial_rows, all_weight_rows, all_quality_rows)
