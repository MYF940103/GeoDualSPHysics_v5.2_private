...
| External-load Terzaghi | Pressure-only baseline works; coupled external-load routes via source `TopLoad` and `AccInput` were unstable for small loads without heavy stabilization. | Current route is experimental only. Need traction/loading strategy and/or better coupled stabilization. |
...
| AccInput-based loading | Preferred over source `TopLoad` for targeted acceleration on a top mkfluid layer. | External-load Terzaghi, triaxial loading experiments. | XML mechanism exists and was tested, but coupled response remains stiff. |
...
| Cleanup of debug interfaces | Keeps production XML/API clean. | All formal cases. | `PorePressureAccelSymCorr` and source `TopLoad*` are candidates after verification. |
...
2. Prefer native `AccInput` or a controlled traction/loading plate over source `TopLoad*`.
...
6. **External loading strategy**
   ...
   - Source `TopLoad*` is deprecated and should not be used in formal cases.
...
- Source-side `TopLoad*` parameters.
...
| Source `TopLoadEnabled`, `TopLoad`, `TopLoadThickness`, `TopLoadRampStart`, `TopLoadRampEnd` | Deprecate now; remove in a dedicated cleanup commit after AccInput/formal loading templates are stable. |
