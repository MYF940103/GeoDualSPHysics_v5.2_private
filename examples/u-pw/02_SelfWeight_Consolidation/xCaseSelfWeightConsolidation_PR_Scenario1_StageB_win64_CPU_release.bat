...
set name=CaseSelfWeightConsolidation_PR_Scenario1_StageB
set rundir=Scenario1_StageB_PR_run
set dirout=%rundir%\%name%_out
set diroutdata=%dirout%\data
set restartdir=Scenario1_StageA_PR_run\CaseSelfWeightConsolidation_PR_Scenario1_StageA_out\data
...
:run
if not exist %restartdir% (
    echo Restart directory not found: %restartdir%
    echo Run Stage A first.
    goto fail
)
...
%dualsphysicscpu% -cpu -mdbc %dirout%\%name% %dirout% -dirdataout data -partbegin:1:1 %restartdir% -sv:csv,binx
if not "%ERRORLEVEL%" == "0" goto fail
