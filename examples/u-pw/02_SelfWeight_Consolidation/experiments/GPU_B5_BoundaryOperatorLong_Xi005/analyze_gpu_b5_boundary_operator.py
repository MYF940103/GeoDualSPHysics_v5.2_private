...
GPU_MODE0_XI005 = ROOT.parent / "GPU_G9b_SelfWeightLong_Xi005"
GPU_XI010 = ROOT.parent / "GPU_G9_SelfWeightLong"
...
def frame_metrics(part: Path, frame_index: int, initial: dict[int, dict[str, float]]) -> dict[str, str]:
    ...
    _, _, dz_mean, _ = settlement(rows, initial)
...
def diff_to(prefix: str, summary: dict[str, str], other: dict[str, str], mapping: dict[str, str]) -> None:
    for local_key, other_key in mapping.items():
        summary[f"{prefix}_{other_key}"] = other.get(other_key, "")
        lv = fget(summary, local_key)
        ov = fget(other, other_key)
        summary[f"b5_minus_{prefix}_{local_key}"] = f"{lv - ov:.12g}" if math.isfinite(lv) and math.isfinite(ov) else ""
...
def main() -> None:
    ...
    with (ROOT / "gpu_b5_boundary_operator_frame_metrics.csv").open("w", newline="") as f:
    ...
    mode0_xi005_rows = read_rows(GPU_MODE0_XI005 / "gpu_g9b_case_summary.csv")
    mode0_xi005 = mode0_xi005_rows[0] if mode0_xi005_rows else {}
    gpu010_rows = read_rows(GPU_XI010 / "gpu_g9_case_summary.csv")
    gpu010 = gpu010_rows[0] if gpu010_rows else {}
    cpu_rows = read_rows(CPU_SW3H / "sw3h_case_summary.csv")
    cpu = cpu_rows[0] if cpu_rows else {}
    peak_excess = max((fget(r, "Excess_maxAbs") for r in frame_rows), default=math.nan)
    final_excess = fget(final, "Excess_maxAbs")
    envelope_ratio = final_excess / peak_excess if peak_excess and math.isfinite(peak_excess) else math.nan
    ...
    summary = {
        ...
        "final_hydrostatic_mean": final.get("hydrostatic_mean", ""),
        ...
        "final_to_peak_Excess_maxAbs_ratio": f"{envelope_ratio:.12g}",
    ...
    diff_mapping = {
        "final_Excess_max": "final_Excess_max",
        "final_Excess_mean": "final_Excess_mean",
        "final_bottom_Excess_mean": "final_bottom_Excess_mean",
        "final_PorePress_max": "final_PorePress_max",
        "final_velocity_max": "final_velocity_max",
        "runtime_seconds": "runtime_seconds",
    }
    if gpu010:
        diff_to("gpu_xi010", summary, gpu010, diff_mapping)
    if mode0_xi005:
        diff_to("gpu_mode0_xi005", summary, mode0_xi005, diff_mapping)
    if cpu:
        diff_to("cpu_sw3h", summary, cpu, diff_mapping)
        cpu_rt = fget(cpu, "runtime_seconds")
        gpu_rt = fget(summary, "runtime_seconds")
        summary["b5_speedup_vs_cpu_sw3h"] = f"{cpu_rt / gpu_rt:.12g}" if gpu_rt else ""

    with (ROOT / "gpu_b5_boundary_operator_case_summary.csv").open("w", newline="") as f:
