...
CASE = "CaseSelfWeightConsolidation_PR_Scenario2_GPU_T3p6_Xi005"
...
def diff_to(prefix: str, summary: dict[str, str], other: dict[str, str], mapping: dict[str, str]) -> None:
    for local_key, other_key in mapping.items():
        ...
        summary[f"g9b_minus_{prefix}_{local_key}"] = f"{lv - ov:.12g}" if math.isfinite(lv) and math.isfinite(ov) else ""
...
def main() -> None:
    ...
    with (ROOT / "gpu_g9b_frame_metrics.csv").open("w", newline="") as f:
    ...
    if cpu:
        ...
        summary["g9b_speedup_vs_cpu_sw3h"] = f"{cpu_rt / gpu_rt:.12g}" if gpu_rt else ""

    with (ROOT / "gpu_g9b_case_summary.csv").open("w", newline="") as f:
