...
set name=CaseCouetteFlow2D_dp001
...
if not exist %dualsphysicsgpu% set dualsphysicsgpu="%dirbinref%/DualSPHysics5.2_GEO_win64.exe"
...
%dualsphysicsgpu% -gpu -mdbc_noslip %dirout%/%name% %dirout% -dirdataout data -svres
if not "%ERRORLEVEL%" == "0" goto fail
...
%measuretool% -dirin %diroutdata% -points PointsVelocity_dp001.txt -onlytype:-all,+fluid -distinter_2h:0.75 -vars:-all,+vel.x,+vel.m -savevtk %dirout2%/PointsVelocity -savecsv %dirout2%/_PointsVelocity
if not "%ERRORLEVEL%" == "0" goto fail
