...
export name=CaseGranularCollapse3D
export dirout=${name}_CPU_out
...
${dualsphysicscpu} -ompthreads:${OMP_NUM_THREADS} ${dirout}/${name} ${dirout} -dirdataout data -svres

export dirout2=${dirout}/particles

${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press,+sigma_kk
