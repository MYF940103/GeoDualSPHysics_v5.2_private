...
set name=CaseNoncohesiveSoilDambreak2D_2650
...
%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartFluid -onlytype:-all,fluid -vars:+idp,+vel,+rhop,+press,+sigma_kk,+sigma_ij,+kplastic,+mk
if not "%ERRORLEVEL%" == "0" goto fail
...
%isosurface% -dirin %diroutdata% -saveslice %dirout2%/Slices_phase1 -onlymk:2 
if not "%ERRORLEVEL%" == "0" goto fail
